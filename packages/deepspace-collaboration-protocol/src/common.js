...
export const COLLABORATION_PROTOCOL = 'deepspace-collaboration.v1';
...
export function createInviteUri(shareId, pairingSecret) {
  ...
  return `deepspace://join/${encodeURIComponent(shareId)}#${pairingSecret}`;
}
...
export function parseInviteUri(uri) {
  ...
  const isNative = parsed.protocol === 'deepspace:' && parsed.hostname === 'join';
