...
export function createRelayServer(options = {}) {
  ...
  const allowedOrigins = parseAllowedOrigins(options.allowedOrigins ?? process.env.DEEPSPACE_RELAY_ALLOWED_ORIGINS);
  const metrics = {
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  console.log(`DeepSpace relay listening on ${host}:${port}`);
