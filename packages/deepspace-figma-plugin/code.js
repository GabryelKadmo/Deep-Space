...
function checkedConnection(value) {
  ...
  if (!loopback || !['http:', 'https:'].includes(url.protocol)) throw new Error('DeepSpace must use a loopback URL.');
...
async function request(path, init = {}) {
  if (!connection) throw new Error('Connect this plugin to DeepSpace first.');
  ...
  if (!response.ok) throw new Error(payload.error || `DeepSpace returned ${response.status}.`);
  return payload.data;
}
...
async function requestBinary(path) {
  ...
  if (!response.ok) throw new Error(`Could not load the DeepSpace asset (${response.status}).`);
...
async function importDesignResources(document) {
  ...
  for (const source of sourceCollections) {
    const name = `DeepSpace / ${document.name} / ${source.name}`;
...
async function importDocument(document) {
  ...
  page.name = `DeepSpace · ${document.name}`;
...
async function synchronizeCurrentFile(document) {
  ...
  if (!link) throw new Error('This DeepSpace design is not linked to the current Figma file.');
...
figma.ui.onmessage = async (event) => {
  try {
    if (event.type === 'connect') {
      ...
      await figma.clientStorage.setAsync('deepspace.connection', connection);
    ...
    if (event.type === 'push-selection') {
      if (!figma.fileKey) throw new Error('Save this Figma file before linking its selection to DeepSpace.');
      ...
      const result = await request(`/api/agent-room/bridge/designs/${encodeURIComponent(event.nodeId)}/figma/selection`, {
        ...
        body: JSON.stringify({
          ...
          summary: 'Import Figma selection through DeepSpace Design Bridge',
        }),
      });
...
figma.clientStorage.getAsync('deepspace.connection').then((saved) => {
  if (saved) {
