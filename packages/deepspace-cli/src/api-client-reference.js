export function apiClientReference() {
  return {
    workflow: [
      ...
      'Use api_client_sync_status before resolving concurrent repository edits. api_client_pull and api_client_push refuse destructive conflicts unless resolution is explicitly filesystem or deepspace.',
      'Use stable unique ids for requests, folders, runners, params, headers, form fields, assertions, and messages.',
      'Secrets are returned as __DEEPSPACE_REDACTED__. Keep that exact value to preserve the stored secret, or provide a replacement value.',
      'Set scriptDialect to bruno, postman, or deepspace and write scripts for that runtime. Scripts are preserved on export; JavaScript is never silently translated between runtimes.',
    ...
    export: {
      ...
      path: 'Must be relative to the workspace or use a registered repository alias, for example .deepspace/exports or @api-tests/bruno.',
    },
    repository: {
      ...
      conflicts: 'api_client_sync_status reports sourceChanged/localChanged/conflict. Pull with resolution=filesystem or push with resolution=deepspace only after reviewing the competing side.',
      persistence: 'The linked collection remains ordinary project files, so git status, commits, CI, Bruno, and Postman see the same format-native requests, scripts, tests, folders, and variables. DeepSpace-only runner configuration remains in the node and lossless native backup.',
    },
    execution: {
