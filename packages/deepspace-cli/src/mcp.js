/**
 * Servidor MCP (Model Context Protocol) do DeepSpace — stdio, JSON-RPC 2.0
...
const DESIGN_COMPONENT_PROPERTY = {
  type: 'object',
  additionalProperties: false,
  properties: {
    id: { type: 'string', format: 'uuid' },
    name: { type: 'string', minLength: 1, maxLength: 120 },
    type: { type: 'string', enum: ['text', 'boolean', 'slot'] },
    targetElementId: { type: 'string', format: 'uuid' },
    defaultValue: { type: ['string', 'boolean', 'null'] },
    preferredValues: { type: 'array', maxItems: 200, items: { type: 'string', format: 'uuid' } },
    order: { type: 'integer', minimum: 0, maximum: 10000 },
  },
  required: ['id', 'name', 'type', 'targetElementId', 'defaultValue'],
};
...
const TOOLS = [
  ...
  { name: 'code_graph_semantic_status', description: 'Retorna o estado do indice semantico local. No modo Assistido, aguarda a sincronizacao automatica da revisao atual sem enviar codigo para fora do dispositivo.', inputSchema: { type: 'object', properties: {} } },
  { name: 'code_graph_semantic_build', description: 'Forca a reconstrucao do indice semantico local. No modo Assistido a atualizacao normal ja e automatica; no Manual esta tool executa a atualizacao explicita. Nao baixa modelo nem envia codigo para fora do dispositivo.', inputSchema: { type: 'object', properties: {} } },
  { name: 'code_graph_semantic_search', description: 'Busca simbolos por intencao no indice semantico local e retorna pontuacao e motivos explicaveis. No modo Assistido, espera a revisao mais recente antes de pesquisar.', inputSchema: { type: 'object', properties: { query: { type: 'string', minLength: 1, maxLength: 120 }, projectId: { type: 'string', format: 'uuid' }, kinds: { type: 'array', items: { type: 'string', enum: ['module', 'namespace', 'class', 'interface', 'type', 'enum', 'function', 'method', 'variable', 'endpoint', 'apiRequest', 'schema', 'gateway', 'resource', 'external'] } }, limit: { type: 'integer', minimum: 1, maximum: 100, default: 30 } }, required: ['query'] } },
  ...
  { name: 'note_create', description: 'Cria uma nota no canvas (conecta somente ao agente autor por padrao).', inputSchema: { type: 'object', properties: { title: { type: 'string' }, content: { type: 'string' }, connect: { type: 'string', description: 'Titulo/id de agente ou "all" para compartilhar explicitamente' } }, required: ['title'] } },
  ...
  { name: 'api_client_reference', description: 'Retorna o contrato, fluxo seguro e exemplos oficiais de scripts Bruno, Postman e DeepSpace para criar colecoes completas sem tentativa e erro.', inputSchema: { type: 'object', properties: {} } },
  ...
  { name: 'api_client_push', description: 'Persiste requests, scripts, testes, ambientes e runners na origem Bruno/Postman vinculada. Recusa mudancas externas; resolution=deepspace confirma explicitamente a substituicao.', inputSchema: { type: 'object', properties: { nodeId: { type: 'string' }, resolution: { type: 'string', enum: ['deepspace'] } }, required: ['nodeId'] } },
  { name: 'api_client_export', description: 'Exporta a colecao conectada para Bruno ou Postman dentro do workspace, preservando estrutura, scripts e testes do runtime escolhido.', inputSchema: { type: 'object', properties: { nodeId: { type: 'string' }, kind: { type: 'string', enum: ['bruno', 'postman'] }, path: { type: 'string', default: '.deepspace/exports' } }, required: ['nodeId', 'kind'] } },
  ...
  { name: 'image_workflow_create', description: 'Cria um fluxo visivel no canvas, conecta este Codex como executor e salva a configuracao inicial sem iniciar a geracao.', inputSchema: { type: 'object', properties: {
    ...
    outputPreset: { type: 'string', enum: ['auto', 'instagram-square', 'instagram-portrait', 'instagram-story', 'tiktok', 'custom'] },
    targetWidth: { type: ['integer', 'null'], minimum: 256, maximum: 3840 }, targetHeight: { type: ['integer', 'null'], minimum: 256, maximum: 3840 },
  ...
  { name: 'image_workflow_validate', description: 'Valida um output individual antes de concluir o run. Confirma PNG integro e alpha real; para tamanho exato, so redimensiona sem recorte quando a proporcao ja corresponde e pede recomposicao nativa ao ImageGen quando ela diverge. O master original e preservado.', inputSchema: { type: 'object', properties: { nodeId: { type: 'string' }, runId: { type: 'string' }, outputPath: { type: 'string' } }, required: ['nodeId', 'runId', 'outputPath'] } },
  ...
  { name: 'design_apply_operations', description: 'Aplica ate 100 operacoes transacionais ao documento. Leia a revisao antes e verifique o resultado depois; divida mutacoes maiores em lotes coerentes para evitar chamadas longas.', inputSchema: { type: 'object', properties: { nodeId: { type: 'string' }, baseRevision: { type: 'number' }, operations: { type: 'array', minItems: 1, maxItems: 100, items: { type: 'object' } }, summary: { type: 'string' }, taskId: { type: 'string' } }, required: ['nodeId', 'baseRevision', 'operations', 'summary'] } },
  ...
  { name: 'design_apply_blueprint', description: 'Aplica uma parte coerente do blueprint aprovado em ate 100 itens. Use chamadas separadas para tokens, bindings/componentes e prototipo/motion, relendo a revisao entre elas.', inputSchema: { type: 'object', properties: {
    ...DESIGN_BATCH_BASE,
    ...
    components: { type: 'array', maxItems: 100, items: { type: 'object', properties: { id: { type: 'string', format: 'uuid' }, name: { type: 'string' }, description: { type: 'string' }, rootElementId: { type: 'string', format: 'uuid' }, setId: { type: ['string', 'null'], format: 'uuid' }, variantValues: { type: 'object' }, properties: { type: 'array', maxItems: 200, items: DESIGN_COMPONENT_PROPERTY }, key: { type: 'string' } }, required: ['id', 'name', 'rootElementId'] } },
  ...
  }, required: ['nodeId', 'baseRevision', 'summary'] } },
  ...
  { name: 'design_figma_sync_preview', description: 'Compara Figma e DeepSpace e classifica alteracoes remotas, locais e conflitos antes de escrever.', inputSchema: { type: 'object', properties: { nodeId: { type: 'string' }, linkId: { type: 'string' } }, required: ['nodeId', 'linkId'] } },
  ...
  { name: 'portal_navigate', description: 'Abre URL no portal identificado por nome unico ou ID.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' }, url: { type: 'string' } }, required: ['nodeId', 'url'] } },
  { name: 'portal_eval', description: 'Disabled for agent safety. Use typed Portal snapshot/click/type/select/extract tools instead; arbitrary scripts cannot inspect authenticated sessions.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' }, js: { type: 'string' } }, required: ['nodeId', 'js'] } },
  { name: 'portal_dom', description: 'Devolve o HTML atual do portal identificado por nome unico ou ID.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' } }, required: ['nodeId'] } },
  { name: 'portal_screenshot', description: 'Captura a tela do portal identificado por nome unico ou ID (base64).', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string', description: 'Nome unico ou ID do portal.' } }, required: ['nodeId'] } },
  { name: 'portal_tabs', description: 'Lista, cria, ativa ou fecha abas da sessao persistente do Portal.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, operation: { type: 'string', enum: ['list', 'new', 'activate', 'close'] }, url: { type: 'string' }, tabId: { type: 'string' } }, required: ['nodeId', 'operation'] } },
  { name: 'portal_snapshot', description: 'Retorna a arvore semantica interativa do Portal com referencias validas ate a proxima snapshot ou alteracao do elemento: e1, e2 etc. Use antes de click/type/select/upload/download.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, interactiveOnly: { type: 'boolean', default: true } }, required: ['nodeId'] } },
  { name: 'portal_click', description: 'Clica em uma referencia semantica obtida por portal_snapshot.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string', pattern: '^e\\d+$' }, button: { type: 'string', enum: ['left', 'middle', 'right'], default: 'left' } }, required: ['nodeId', 'ref'] } },
  { name: 'portal_type', description: 'Digita em uma referencia semantica; pode limpar o valor e enviar Enter.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, text: { type: 'string' }, clear: { type: 'boolean', default: true }, submit: { type: 'boolean', default: false } }, required: ['nodeId', 'ref', 'text'] } },
  { name: 'portal_select', description: 'Seleciona um ou mais valores em um campo do Portal.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, values: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['nodeId', 'ref', 'values'] } },
  { name: 'portal_upload', description: 'Envia arquivos confinados ao workspace para um input file referenciado.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, paths: { type: 'array', items: { type: 'string' }, minItems: 1 } }, required: ['nodeId', 'ref', 'paths'] } },
  { name: 'portal_download', description: 'Aciona um download e o confina a pasta configurada dentro do workspace.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, filename: { type: 'string' } }, required: ['nodeId', 'ref'] } },
  { name: 'portal_wait', description: 'Aguarda elemento, texto, URL ou atraso limitado no Portal.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, ref: { type: 'string' }, text: { type: 'string' }, urlIncludes: { type: 'string' }, delayMs: { type: 'number' } }, required: ['nodeId'] } },
  { name: 'portal_extract', description: 'Extrai texto, links, tabela ou atributo de forma limitada e estruturada.', inputSchema: { type: 'object', properties: { taskId: { type: 'string', description: 'Active task assigned to this terminal; required when several tasks are active.' }, nodeId: { type: 'string' }, kind: { type: 'string', enum: ['text', 'links', 'table', 'attribute'], default: 'text' }, ref: { type: 'string' }, attribute: { type: 'string' } }, required: ['nodeId'] } },
  ...
  { name: 'device_screenshot', description: 'Salva um screenshot no diretorio .deepspace do workspace.', inputSchema: { type: 'object', properties: {} } },
  ...
  { name: 'recruit', description: '(maestro) Recruta agente novo no canvas. Para composição visual, prefira effort medium ou high; xhigh aumenta muito a latência de payloads estruturados.', inputSchema: { type: 'object', properties: { title: { type: 'string' }, provider: { type: 'string', description: 'Id de um provider registrado no DeepSpace.' }, profile: { type: 'string', description: 'Nome de um perfil de multi-conta desse provider (ver usage). Sem isso, usa a conta padrão.' }, model: { type: 'string' }, effort: { type: 'string', enum: ['low', 'medium', 'high', 'xhigh', 'max', 'ultra'] }, role: { type: 'string' }, floorId: { type: 'string', description: 'Andar ativo onde o agente deve trabalhar.' } }, required: ['title'] } },
...
async function callTool(bridge, findFreePort, selfAgent, name, args = {}) {
  ...
  const computerCommand = (input) => {
    if (!selfAgent) throw new Error('Computer actions require an active DeepSpace terminal identity.');
  ...
  switch (name) {
    ...
    case 'tool_propose': {
      if (!selfAgent) throw new Error('tool_propose exige identidade de um terminal DeepSpace ativo.');
      const { taskId, ...input } = args;
      return bridge('POST', '/api/agent-room/bridge/tools', { operation: 'propose', from: selfAgent, taskId, input });
    }
    case 'tool_update': {
      if (!selfAgent) throw new Error('tool_update exige identidade de um terminal DeepSpace ativo.');
    ...
    case 'tool_execute': {
      if (!selfAgent) throw new Error('tool_execute exige identidade de um terminal DeepSpace ativo.');
      const { toolId, taskId, ...input } = args;
    ...
    case 'api_client_export':
      if (!selfAgent) throw new Error('identidade do agente desconhecida (DEEPSPACE_NODE_ID ausente).');
      return bridge('POST', `/api/agent-room/bridge/api-clients/${encodeURIComponent(args.nodeId)}/export`, { kind: args.kind, path: args.path ?? '.deepspace/exports', from: selfAgent });
    ...
    case 'design_apply_operations':
      if (args.operations.length > 100) throw new Error('design_apply_operations accepts at most 100 operations. Split the change into coherent revision-checked batches.');
      return bridge('PATCH', `/api/agent-room/bridge/designs/${encodeURIComponent(args.nodeId)}`, {
        baseRevision: args.baseRevision,
    ...
    case 'design_apply_blueprint': {
      if ((args.elements?.length ?? 0) > 0 && !args.pageId) {
        throw new Error('design_apply_blueprint requires pageId when creating elements. Token, component, prototype, motion, and presentation batches do not require a page.');
      }
      ...
      const operations = [
        ...
        ...(args.components ?? []).map((component) => ({ kind: 'add-component', component: { ...component, description: component.description ?? '', setId: component.setId ?? null, variantValues: component.variantValues ?? {}, properties: (component.properties ?? []).map((property, order) => ({ ...property, preferredValues: property.preferredValues ?? [], order: property.order ?? order })), key: component.key ?? component.id, updatedAt: now } })),
      ...
      if (operations.length > 100) throw new Error('design_apply_blueprint accepts at most 100 total items. Apply tokens, bindings/components, and prototype/motion as separate revision-checked batches.');
      return bridge('PATCH', `/api/agent-room/bridge/designs/${encodeURIComponent(args.nodeId)}`, {
        baseRevision: args.baseRevision,
        operations,
        summary: args.summary,
    ...
    case 'portal_create': {
      if (!selfAgent) throw new Error('identidade do agente desconhecida (DEEPSPACE_NODE_ID ausente) — crie o portal pelo canvas ou pela CLI dentro do terminal de um agente.');
    ...
    case 'portal_navigate':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'navigate', args: { url: args.url }, from: selfAgent });
    case 'portal_eval':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'eval', args: { js: args.js } });
    case 'portal_dom':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'dom', args: {}, from: selfAgent });
    case 'portal_screenshot':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'screenshot', args: {}, from: selfAgent });
    case 'portal_tabs':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'tabs', args: { operation: args.operation, ...(args.url ? { url: args.url } : {}), ...(args.tabId ? { tabId: args.tabId } : {}) }, from: selfAgent });
    case 'portal_snapshot':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'snapshot', args: { interactiveOnly: args.interactiveOnly !== false }, from: selfAgent });
    case 'portal_click':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'click', args: { ref: args.ref, button: args.button ?? 'left' }, from: selfAgent });
    case 'portal_type':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'type', args: { ref: args.ref, text: args.text, clear: args.clear !== false, submit: args.submit === true }, from: selfAgent });
    case 'portal_select':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'select', args: { ref: args.ref, values: args.values }, from: selfAgent });
    case 'portal_upload':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'upload', args: { ref: args.ref, paths: args.paths }, from: selfAgent });
    case 'portal_download':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'download', args: { ref: args.ref, ...(args.filename ? { filename: args.filename } : {}) }, from: selfAgent });
    case 'portal_wait':
      return bridge('POST', '/api/agent-room/bridge/portal', { taskId: args.taskId, nodeId: args.nodeId, action: 'wait', args: { ...(args.ref ? { ref: args.ref } : {}), ...(args.text ? { text: args.text } : {}), ...(args.urlIncludes ? { urlIncludes: args.urlIncludes } : {}), ...(args.delayMs !== undefined ? { delayMs: args.delayMs } : {}) }, from: selfAgent });
    case 'portal_extract':
    ...
    case 'status': {
      if (!selfAgent && !args.taskId) throw new Error('identidade do agente desconhecida (DEEPSPACE_NODE_ID ausente e nenhuma tarefa atribuída foi informada).');
      return bridge('POST', '/api/agent-room/bridge/activity', {
        ...(selfAgent ? { from: selfAgent } : {}),
        state: args.state,
        action: args.action,
        taskId: args.taskId,
      });
    ...
    case 'recruit': {
      if (!selfAgent) throw new Error('identidade do agente desconhecida (DEEPSPACE_NODE_ID ausente) — recruit so funciona dentro do terminal do maestro.');
    ...
    case 'dismiss': {
      if (!selfAgent) throw new Error('identidade do agente desconhecida (DEEPSPACE_NODE_ID ausente) — dismiss so funciona dentro do terminal do maestro.');
    ...
    try {
      if (message.method === 'initialize') {
        reply({
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'deepspace', version },
        });
