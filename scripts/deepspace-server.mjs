...
if (process.env.DEEPSPACE_DATA_DIR) {
  const dataDir = process.env.DEEPSPACE_DATA_DIR;
  mkdirSync(dataDir, { recursive: true });
  const dbPath = process.env.DB_PATH ?? 'database.db';
  if (!isAbsolute(dbPath)) process.env.DB_PATH = resolve(dataDir, dbPath);
}

// Shim da CLI `deepspace` no PATH dos terminais (ver install-deepspace-shim.mjs).
...
server.listen(port, host, () => {
  ...
  writeDeepSpaceRuntimeFile({
    apiUrl: `http://${host}:${port}`,
    coreId: process.env.DEEPSPACE_CORE_ID ?? null,
    coreToken: process.env.DEEPSPACE_CORE_TOKEN ?? null,
    pid: process.pid,
    startedAt: process.env.DEEPSPACE_CORE_STARTED_AT ?? new Date().toISOString(),
    version: process.env.DEEPSPACE_CORE_VERSION ?? null,
  });
});
...
async function shutdown() {
  ...
  await globalThis.__deepspaceShutdownCollaboration?.().catch(() => undefined);
  await globalThis.__deepspaceShutdownDevices?.().catch(() => undefined);
  wss.close();
