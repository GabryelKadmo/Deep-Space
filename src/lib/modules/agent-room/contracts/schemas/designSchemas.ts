...
const designInstanceElementOverridesSchema = z.object({
  ...
  lineHeight: z.number().finite().min(4).max(2_000).nullable().optional(),
  letterSpacing: z.number().finite().min(-100).max(1_000).optional(),
  paragraphSpacing: z.number().finite().min(0).max(10_000).optional(),
  textAlign: z.enum(['left', 'center', 'right']).optional(),
  textVerticalAlign: z.enum(['top', 'middle', 'bottom']).optional(),
  textDecoration: z.enum(['none', 'underline', 'line-through']).optional(),
  textTransform: z.enum(['none', 'uppercase', 'lowercase', 'capitalize']).optional(),
  textAutoResize: z.enum(['fixed', 'height', 'width-height']).optional(),
  layoutMode: z.enum(['none', 'horizontal', 'vertical', 'grid']).optional(),
  layoutWrap: z.boolean().optional(),
  layoutGap: z.number().finite().min(0).max(10_000).optional(),
  layoutRowGap: z.number().finite().min(0).max(10_000).optional(),
  layoutColumnGap: z.number().finite().min(0).max(10_000).optional(),
  layoutPaddingTop: z.number().finite().min(0).max(10_000).optional(),
  layoutPaddingRight: z.number().finite().min(0).max(10_000).optional(),
  layoutPaddingBottom: z.number().finite().min(0).max(10_000).optional(),
  layoutPaddingLeft: z.number().finite().min(0).max(10_000).optional(),
  layoutGridColumns: z.number().int().min(1).max(64).optional(),
  layoutAlign: z.enum(['start', 'center', 'end', 'space-between']).optional(),
  layoutCrossAlign: z.enum(['start', 'center', 'end', 'stretch']).optional(),
  layoutItemAbsolute: z.boolean().optional(),
  widthSizing: z.enum(['fixed', 'hug', 'fill']).optional(),
  heightSizing: z.enum(['fixed', 'hug', 'fill']).optional(),
  minWidth: z.number().finite().min(1).max(100_000).nullable().optional(),
  maxWidth: z.number().finite().min(1).max(100_000).nullable().optional(),
  minHeight: z.number().finite().min(1).max(100_000).nullable().optional(),
  maxHeight: z.number().finite().min(1).max(100_000).nullable().optional(),
...
export const designElementSchema = z.object({
  ...
  fontFamily: z.string().trim().min(1).max(160).default('Inter Variable'),
  fontSize: z.number().finite().min(4).max(1000).default(16),
  fontWeight: z.number().int().min(100).max(900).default(400),
  fontStyle: z.enum(['normal', 'italic']).default('normal'),
  lineHeight: z.number().finite().min(4).max(2_000).nullable().default(null),
  letterSpacing: z.number().finite().min(-100).max(1_000).default(0),
  paragraphSpacing: z.number().finite().min(0).max(10_000).default(0),
  textAlign: z.enum(['left', 'center', 'right']).default('left'),
  textVerticalAlign: z.enum(['top', 'middle', 'bottom']).default('top'),
  ...
  textAutoResize: z.enum(['fixed', 'height', 'width-height']).default('fixed'),
  ...
  layoutCrossAlign: z.enum(['start', 'center', 'end', 'stretch']).default('start'),
  layoutItemAbsolute: z.boolean().default(false),
  widthSizing: z.enum(['fixed', 'hug', 'fill']).default('fixed'),
  heightSizing: z.enum(['fixed', 'hug', 'fill']).default('fixed'),
  minWidth: z.number().finite().min(1).max(100_000).nullable().default(null),
  maxWidth: z.number().finite().min(1).max(100_000).nullable().default(null),
  minHeight: z.number().finite().min(1).max(100_000).nullable().default(null),
  maxHeight: z.number().finite().min(1).max(100_000).nullable().default(null),
...
export const designOperationSchema = z.discriminatedUnion('kind', [
  ...
  z.object({ kind: z.literal('apply-auto-layout'), frameId: z.string().uuid() }),
  z.object({
