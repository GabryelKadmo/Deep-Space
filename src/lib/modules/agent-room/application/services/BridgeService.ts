...
Você está rodando dentro de um workspace do Deep Space. A CLI \`orkestrai\` dá acesso à ponte.
...
- Tool MCP \`api_client_reference\` (ou \`orkestrai api reference\`) — consulte antes de autorar uma coleção: retorna o contrato completo e exemplos de testes/variáveis nos runtimes Bruno, Postman e Deep Space.
...
`;
  ...
  private agentsMdBlock(): string {
    return [
      ...
      '',
      'Este projeto roda dentro de um workspace do Deep Space. Você tem a CLI `orkestrai` e/ou tools MCP `orkestrai` disponíveis para colaborar com o time no canvas:',
