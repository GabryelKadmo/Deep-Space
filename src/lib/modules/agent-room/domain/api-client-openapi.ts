  ...
  for (const [path, rawPathItem] of Object.entries(document.paths ?? {})) {
    ...
    for (const method of HTTP_METHODS) {
      ...
      requests.push(apiClientRequestSchema.parse({
        ...
        ...imported,
        preRequestScript: text(operation['x-deepspace-pre-request'] ?? operation['x-pre-request-script']),
        postResponseScript: text(operation['x-deepspace-post-response'] ?? operation['x-post-response-script']),
