...
export type CanvasNodeType = 'terminal' | 'console' | 'note' | 'fileTree' | 'git' | 'editor' | 'diff' | 'portal' | 'apiClient' | 'loop' | 'group' | 'shape' | 'tasks' | 'flow' | 'image' | 'imageWorkflow' | 'usage' | 'codeGraph' | 'controlCenter' | 'reviewCenter' | 'workstreams' | 'memory' | 'annotations' | 'huddles' | 'automation' | 'toolWorkshop' | 'device' | 'computer' | 'design';
