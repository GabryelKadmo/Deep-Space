  ...
  sections: [
    ...
    },
    {
      ...
      bullets: [
        ...
        'Imported scripts are preserved and executed by their selected source runtime, so no JavaScript transpilation is needed. Postman team Package Library, hosted datasets, mocks, and other cloud-owned state require Postman services; they are not contained in a portable collection file. Bruno stays in its official safe QuickJS runtime: unsafe NodeVM access to the host filesystem, processes, and arbitrary local modules is deliberately disabled. The native .orkestrai-api.json format remains the lossless backup for Deep Space-only state.',
...
Header: Authorization = Bearer {{accessToken}}`,
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Open Roles, choose "Discover from another folder...", and select the project that owns the role. Deep Space validates the bounded role files under that project\'s .orkestrai/roles directory, imports only new role names, and never overwrites an existing workspace role.',
    ...
    },
    {
      ...
      body: 'Add MCP servers in the workspace editor (e.g.: filesystem, web, database) — agents get the tools natively, and Deep Space itself appears as an MCP server with the canvas actions (orkestrai mcp). Presets can carry the MCPs along with the team.',
  ...
  changelog: [
{
  ...
  "items": [
    ...
    "The agent bridge is unchanged on purpose: the orkestrai command, the .orkestrai/ directory, and the environment variables stay as they are, because renaming them would break existing workspaces."
  ]
},
{
...
},
{
  ...
  "items": [
    ...
    },
    {
      ...
      items: [
        'Added a "Discover from another folder..." button next to Roles\' existing repository discovery: pick any folder in a native dialog and Deep Space imports every `role.json` found under its `.orkestrai/roles/` directory.',
