...
export const DOCS_EN: DocsCatalog = {
  quickstart: [
    ...
    'Talk to any agent through its own terminal, or let the leader distribute everything on its own via the deepspace CLI.',
  ],
  sections: [
    {
      id: 'workspaces',
...
console.log('Created user:', body.id);`,
  ...
  expect(res.getBody()).to.have.property('id');
        ...
        {
          id: 'deepspace-native',
          title: 'Native Deep Space: assertions and JavaScript',
...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: "Open a Portal and sign in yourself. In its gear choose Manual only, Read only, or Read and interact, then select the exact agents. The agent controls the same native page and tabs visible in Canvas or Workbench; opening another view does not create a second browser session. Visible control is the default. Enable background control explicitly to continue with that Portal hidden, and use Pause to stop further brokered actions immediately. Agent mutations also require an enforcing policy in Automations > Security and an active assigned task. Set normal risk categories to preapproved only within the perimeter you intend to authorize; unknown clicks are conservatively gated. Password, token, OTP, and marked-private fields are withheld from semantic snapshots and DOM reads and masked in captures; arbitrary agent JavaScript is disabled. Re-snapshot after changes instead of reusing stale element references. In Security, enable Agent-created tools only for selected agents and executor types in Bounded mode. Valid fixtures, limits, existing Portal or integration permissions, and destination-bound SecretRefs are required. Commands still require owner publication. Browser tools bind to a Portal id and use ordered typed steps with exact semantic role/name targets and {{input}} templates. Gate approvals resume the same tool run from saved completed steps using its original revision and idempotency key; do not invent a new key for a retry. An interrupted effect whose outcome is unknown is not automatically replayed. Audit expands operation, actor, target, risk decision, safe output, and correlation details and refreshes without overwriting policy edits. Emergency stop remains latched even if the policy switch is turned off; prepare and save an explicit resume. Webhook triggers live in Automations > New > Webhook; after saving, reopen the editor to see the complete local endpoint and send X-Deep Space-Webhook-Secret or Bearer authentication. Outbound webhooks are configured separately in Integrations. Internet ingress needs a private tunnel or a proxy limited to the webhook route, never public access to the entire Core. Browser permissions are broker controls, not an OS sandbox: a free shell, another browser, or unmediated program can act outside this audit. The app must remain running and the computer awake for local 24/7 work.",
    },
    {
      id: 'tool-workshop',
    ...
    },
    {
      ...
      body: 'In Code Graph, open Insights, then Runtime. Select one indexed repository and provide a path relative to that repository. Auto-detection accepts LCOV .info, JUnit XML, common stack traces, or the bounded Deep Space JSON v1 contract below. JSON paths are also repository-relative and line numbers are one-based. Files are limited to 5 MB; coverage is capped at 50,000 locations and failures or calls at 5,000 relationships. Deep Space stores only content hashes, counters, mapped symbol ids, paths, line numbers, and derived relationships. It never persists the raw file, stack text, test output, environment values, or source bodies.',
      bullets: [
        ...
        'Use deepspace graph evidence import <projectId> <relativePath> --kind auto from an agent terminal, or the equivalent code_graph_evidence_import MCP tool.',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Enable Run Deep Space in the background under Settings → Autonomy & 24/7 Core, set the watcher terminal to Persistent under Agent runtime, and define its concurrency and provider-usage safeguards. Then create a routine every 1–5 minutes on the leader: "check the board (deepspace task list); assign whatever is unassigned; if an agent is missing, recruit". Closing the window keeps the supervised process and durable runs available in the tray; Quit Deep Space stops them explicitly.',
    ...
    },
    {
      ...
      body: 'Hourly routine on a shell or agent: "run the tests; if they fail, open a task for the team and notify me (deepspace notify)". You get a native system notification and the kanban already has the card.',
    ...
    },
    {
      ...
      body: 'Open Roles, choose "Discover from another folder...", and select the project that owns the role. Deep Space validates the bounded role files under that project\'s .deepspace/roles directory, imports only new role names, and never overwrites an existing workspace role.',
    ...
    },
    {
      ...
      title: 'Contribute to Deep Space with three perspectives',
      body: 'Apply Deep Space Contributing. Claude leads, Codex and Kimi act as independent oracles and both must approve the plan before any task is created; Svelar, desktop, and QA/release specialists execute the documented plan.',
    ...
    },
    {
      ...
      body: 'Add MCP servers in the workspace editor (e.g.: filesystem, web, database) — agents get the tools natively, and Deep Space itself appears as an MCP server with the canvas actions (deepspace mcp). Presets can carry the MCPs along with the team.',
      tags: ['MCP', 'typed tools', '.mcp.json'],
    },
    {
      id: 'managed-ports',
      title: 'Release ports left by dev servers',
      body: 'Create a local Portal for the app (e.g. http://localhost:5173). The Ports panel, immediately after Usage in the bottom toolbar, shows whether that listener is active, which process/PID owns it, and lets you stop it with confirmation. Only ports linked to local Portals in the workspace are listed; Deep Space\'s own server is protected.',
    ...
    },
    {
      ...
      body: 'Add the Usage node to the canvas, set Claude as the source and Codex as the fallback, then choose the 5-hour, weekly, or monthly window and its percentage. Before delegating new work, the leader checks deepspace usage and recommends the healthy agent when the source crosses that threshold. The panel also explains why Antigravity, Cursor, Devin, OpenCode, or Cline cannot provide the same automatic percentage and links to each official source instead of guessing; conversations and tasks already in progress stay on their current provider.',
      tags: ['Canvas usage', 'fallback', 'delegation'],
    },
    {
      id: 'organize-canvas',
      title: 'Reorganize a growing workspace',
      body: 'Choose Organize canvas from the toolbar or command palette. Deep Space lays out the complete visible canvas into deterministic rows without moving nodes into each other, even when one item was still selected before the command; groups move as intact units and keep their internal arrangement. New agent, note, Portal, board, image workflow, reference, and generated Image nodes also search the real occupied rectangles and keep a stable margin instead of landing on existing work. Connections stay behind every node.',
    ...
    },
    {
      ...
      body: 'Capture the current workspace as a custom Team Pack, publish a semantic version with release notes, and inspect its immutable checksums. Export the pack for another installation or import a shared file after Deep Space validates its schema, size, content, and SHA-256 checksum. Agents, roles, skills, stages, routines, MCP configuration, and layout travel; live sessions and credentials do not.',
    ...
    },
    {
      ...
      body: 'Drag an image, PDF, file, or HTTP/HTTPS link onto a briefing note, an agent composer, or a kanban card. Deep Space stores files up to 10 MB inside the workspace, inserts a readable reference, and delivers the title, description, and every attachment when the lead or agent receives the task.',
  ...
  changelog: [
{
  ...
  "summary": "A state marker on the icon, and edit and delete always in the same column",
  "items": [
    "The workspace rows in the sidebar had no shared shape: the state marker (hibernating, or the dot for live sessions) sat inside the name, so it pushed the edit and delete buttons around and every row ended them at a different place. The marker moved to the corner of the workspace icon, and the two actions now sit in a fixed pair, in the same column on every row. Long names truncate instead of shoving the buttons out.",
...
},
{
  ...
  "items": [
    "The canvas zoom control was rebuilt as a single row — minus, the current percentage with a preset menu, plus, a separator, fit to screen and a canvas lock. The xyflow control it replaces is a vertical stack of square buttons, and inside the new band it left an empty strip under itself. The percentage menu jumps straight to 50/75/100/150/200% or fits the whole canvas, and the lock freezes panning, dragging and selection so a crowded canvas can be read without moving it by accident.",
    "The task detail opens as a drawer inside the kanban node instead of a centered modal. A modal covered the board it came from, so the column a card belongs to — the reason the card is being read — disappeared behind it. Escape or the back arrow returns to the board, and Previous/Next still walk the column.",
    "Settings shows a floating bar as soon as something is edited: \"Careful — you have unsaved changes!\", with Reset and Save changes. The dialog it replaces only appeared when leaving the page, so an edit could sit unsaved indefinitely with nothing on screen saying so. Trying to leave with changes pending still cancels the navigation, and now flashes the bar instead of opening a dialog; saving or resetting from it completes the navigation that was blocked.",
    "On Windows the canvas had no share button at all. The custom title bar takes over share and notifications from the sidebar there, but it only rendered share on the Workbench, so on the canvas the button was in neither place. The title bar now shows it on both, and the canvas registers the same handler that opens the sharing dialog.",
    "Linking two nodes no longer depends on hitting a 20px target: releasing the rope anywhere over the destination node connects it, and the connection radius around each dot is wider. xyflow only reports a destination when the rope ends within a dot’s radius, and the middle of a node is far outside every one of them, so the rope simply vanished. The connection dots on every node also stay visible for as long as a rope is being dragged — previously the destination node showed nothing, because its dots only appeared on hover and the pointer was captured by the drag.",
    "A node can no longer be roped to itself, and two nodes that are already linked no longer stack a second rope on top of the first. Dropping the rope back on its own node, or redoing a link that already existed, silently created edges that overlapped in the same place and had to be deleted one at a time. The guard lives in the workspace service, so the bridge (deepspace ask, recruit) is covered too.",
    "A Portal marked itself unavailable — and, since the error notice hides the page, went blank — whenever a navigation was replaced by another one. A page redirecting on load rejects the first navigation with ERR_ABORTED, which is not a failure; the failure handler already ignored that code and the load call now reads it the same way.",
    "Usage lists a manually added profile right after the provider it belongs to, instead of at the end of the list, and each extra account carries its own name under the provider name, so two Claude accounts standing side by side are told apart at a glance. The leader-routing panel also lost the misaligned field: the window hint sat inside the grid and stretched a column."
  ]
},
{
...
},
{
  ...
  "items": [
    "Product name, application id, installer, shortcuts, documentation, and interface strings in all three languages now use Deep Space. Attribution to the upstream project stays in NOTICE, LICENSE, and the README. The package description is now the product tagline, which also feeds the summary shown by the installers.",
    "Auto-update and release automation target the Deep Space repository, so an installed build never offers the original application over itself.",
    "The sharing relay endpoint is now configurable through PUBLIC_RELAY_URL, with the deployment walkthrough in docs/relay.md.",
    "The agent bridge was renamed as well: the command is deepspace, the folder is .deepspace/, the environment variables are DEEPSPACE_ and the protocol is deepspace:. It had been left on the old naming to protect workspaces from earlier installs; with no published release, there were none to protect. A workspace created before the rename is repaired on first open.",
    "The default theme is monochrome on near black. Accent is white, and page, canvas and sidebar share one tone, so panels no longer read as grey bars against the content. Muted text was lightened to clear the minimum contrast ratio, and the light theme took the neutral equivalent.",
...
},
{
  ...
  "items": [
    ...
    "An image or file pasted into a terminal becomes a workspace attachment under .deepspace/attachments/ and its path is pasted into the prompt, the same contract the canvas composer already uses to hand references to agents.",
    "Alt+V stays free for CLIs that document their own shortcut."
  ]
},
{
  "date": "September 11, 2026 · 0.30.0",
  "title": "Deep Space 0.30.0: natural-language desktop work and signed microphone access",
  ...
  "title": "Deep Space 0.29.0: Persistent agents, visible browser workflows, and reusable tools",
  ...
  "items": [
    ...
    "Closing every window keeps enabled active work in the system tray; Quit Deep Space remains the explicit command that stops the Core and its child processes.",
...
},
    {
      ...
      title: 'Deep Space 0.28.0: exact social delivery without unsafe cropping',
      ...
      items: [
        ...
        'ImageGen receives a measurable safe area for the selected frame. Deep Space preserves the native master and resamples the complete image only when its aspect ratio already matches; incompatible results return to native ImageGen for safe recomposition or outpainting instead of cropping content.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.27.0: the Design editor starts to feel like a design tool',
    ...
    },
    {
      ...
      title: 'Deep Space 0.26.1: reliable Git Floors on WSL',
    ...
    },
    {
      ...
      title: 'Deep Space 0.26.0: a professional native Design workspace',
      ...
      items: [
        ...
        'Windows packages include a pinned console Node runtime verified by SHA-256. Native and WSL bridge launchers use a real console process instead of the graphical Deep Space.exe, preserving stdout, stderr, MCP JSON-RPC, and confirmed ask replies; existing WSL workspaces repair their launcher lazily.',
        ...
        'API Client HTML visualizers resolve relative media and styles against the credential-free request URL instead of Deep Space\'s local server, under a bounded sandbox that blocks active content.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.25.0: a complete Remote workspace on every screen',
    ...
    },
    {
      ...
      title: 'Deep Space 0.24.2: confirmed agent message delivery on Windows',
      ...
      items: [
        ...
        'A terminal redraw no longer counts as delivery: Deep Space confirms the exact prompt in the provider transcript and retries only Enter within a bounded window when needed.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.24.1: secure dependencies and CI-gated releases',
    ...
    },
    {
      ...
      title: 'Deep Space 0.24.0: move complete Canvas selections between workspaces',
    ...
    },
    {
      ...
      title: 'Deep Space 0.23.0: operational code intelligence and reliable continuity',
      ...
      items: [
        ...
        'Restored MCP and CLI sessions without DEEPSPACE_NODE_ID can report Control Center state through a task assigned to their real workspace agent; calls without either verified identity source remain blocked.',
        ...
        'The reproducible code-graph benchmark validates the Deep Space repository, a temporary full mirror, and a synthetic repository against the documented SQLite search, traversal, and single-file incremental gates; the real 1,672-file mirror stays below the two-second incremental target, so no alternate graph database is justified.',
        'Workspace and preset instructions now use a bounded managed block in AGENTS.md and, when enabled, CLAUDE.md. Existing user content is preserved, legacy Deep Space-owned files migrate safely, and disabling synchronization removes only that block.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.22.0: native image workflows for people and agents',
      ...
      items: [
        ...
        'Deep Space never asks for or stores an image API key and never calls a provider endpoint directly. Only the assigned Codex can complete the run, and exact paths, signatures, sizes, and public errors are bounded before persistence.',
        ...
        'New or recovered agents never resume another terminal\'s latest conversation: Deep Space resumes only the exact ID attributed to the node and starts a clean conversation when that link does not exist yet.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.21.2: reliable terminal paste on Windows',
      ...
      items: [
        ...
        'Clipboard contents stay inside Electron and are never exposed through the renderer bridge; only the focused Deep Space window can request the paste.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.21.1: workspaces stay paused and Windows stays in the background',
    ...
    },
    {
      ...
      title: 'Deep Space 0.21.0: portable Roles and faster workspace setup',
      summary: 'Reuse specialist Roles, select the right provider account, and file a workspace into its folder during creation.',
      items: [
        'Added a "Discover from another folder..." button next to Roles\' existing repository discovery: pick any folder in a native dialog and Deep Space imports every `role.json` found under its `.deepspace/roles/` directory.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.20.1: safe Codex MCP configuration',
      summary: 'Codex keeps the automatic workspace bridge without surrendering control of its global dotfile or Git visibility.',
      items: [
        'Codex receives the Deep Space and official Figma MCP definitions through ephemeral native and WSL launch overrides; workspace provisioning no longer rewrites ~/.codex/config.toml.',
        'The exact malformed multiline args and duplicate env structure written by older Deep Space builds is repaired after validation, with a backup, serialized access, and atomic replacement; unrelated malformed TOML stays untouched.',
        'AGENTS.md, provider MCP files, and opencode.json are no longer hidden through .git/info/exclude; exact legacy blocks are narrowed to Deep Space-owned runtime and skill directories.',
        'Bridge provisioning failures now enter desktop diagnostics instead of disappearing silently.',
      ],
    },
    {
      date: 'Aug 25, 2026 · 0.20.0',
      title: 'Deep Space 0.20.0: organized workspaces and sharper provider tools',
      ...
      items: [
        'MCP servers and skills added or installed on a workspace now propagate to Cursor, Cline, Devin, Antigravity, and OpenCode’s native config formats, matching the same providers the built-in Deep Space bridge already covers.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.19.0: provider accounts, status, identity, and clearer routing',
      ...
      items: [
        ...
        'The PTY WebSocket accepts browser connections only from Deep Space on the exact application port, preventing another localhost website from opening or controlling terminal sessions.',
      ],
    },
    {
      date: 'Aug 24, 2026 · 0.18.1',
      title: 'Deep Space 0.18.1: reliable project, Portal, voice, and terminal state',
      summary: 'Project environments remain isolated while desktop browsing, dictation, and terminal rendering recover reliably.',
      items: [
        'Terminal processes keep the user operating-system environment and Deep Space bridge, but remove the desktop APP_KEY and every private variable loaded from the app runtime. Laravel encrypted records, cookies, and sessions therefore use the project .env and no longer fail with “The MAC is invalid”.',
        'Portal login pop-ups now open in a sandboxed Deep Space window with the same persistent session instead of escaping to the system browser. Cookies and storage are flushed to disk, and each Portal node restores its last navigated URL.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.18.0: durable coordination, sourced knowledge, and reusable teams',
    ...
    },
    {
      ...
      title: 'Deep Space 0.17.0: complete API test authoring for people and agents',
      summary: 'Runtime-aware JavaScript tests, completion, and protected MCP/CLI authoring now share one collection model.',
      items: [
        'The Tests tab now switches between structured assertions and a full-height JavaScript editor with contextual completion for Bruno, Postman, and native Deep Space APIs. Test scripts execute separately from post-response automation and round-trip through Bruno and Postman exports.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.16.0: official Postman and Bruno scripting runtimes',
      ...
      items: [
        ...
        'Documentation now includes a complete, searchable API Client scripting reference with separate copyable examples for Postman Runtime, Bruno QuickJS, and native Deep Space declarative tests, plus the explicit boundary around Postman cloud-only services.',
      ],
    },
    {
      date: 'Aug 20, 2026 · 0.15.0',
      title: 'Deep Space 0.15.0: reusable commands and a multi-protocol API Client',
      ...
      items: [
        ...
        'API collections can now export to Bruno through its official serializer or to Postman v2.1 while retaining REST metadata not directly editable in Deep Space. A versioned Deep Space format restores the complete native state, including folders, runners, environments, scripts, and history.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.14.0: native RPM packages for Linux',
      summary: 'Fedora, RHEL, CentOS, and compatible distributions now have a package-native Deep Space installer.',
      items: [
        ...
        'RPM files use the same stable Deep Space product naming as the other installers.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.13.0 keeps terminal keyboard input isolated from Canvas accessibility shortcuts. Escape correctly reaches Vim, merge/rebase editors, pagers, and other TUIs without deselecting the node or blurring xterm; search and dictation remain local to the terminal.',
        ...
        'Native shell terminals now preserve their current folder after restarting Deep Space. Cursor and other providers also receive an explicit tool to list existing notes before reading or editing, preventing duplicates and empty-array dead ends.',
        ...
        'Native Design Mode now provides typed design variables in collections and modes, aliases, searchable property bindings, instant mode previews, and the full revision-safe design command bus for agents through the Deep Space MCP.',
    ...
    },
    {
      ...
      items: [
        ...
        'When creating or editing a WSL workspace, Deep Space now derives and locks the Windows-visible folder from the Linux path automatically, without requiring a second equivalent path or rejecting the configuration by mistake.',
      ],
    },
    {
      ...
      items: [
        ...
        'Workspace sharing now offers separate Browser/mobile and Deep Space app invitations. The installable Remote PWA follows agents, tasks, reviews, activity, and provider usage, persists a non-extractable WebCrypto key, and removes the invitation secret from the URL before connecting.',
        'Workspace sharing now uses the production relay.deepspace.app endpoint by default. The relay accepts the installed app\'s dynamic local origin and configured official web origins while rejecting unrelated websites.',
        ...
        'Images, PDFs, files, and links can now be dropped, pasted, or selected in agents, tasks, notes, and composers; files up to 10 MB stay confined to .deepspace/attachments/ and travel with the complete brief.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.8.3: the searchable model selector now follows the official shadcn-svelte composition, opens without clipping search, and keeps its icon, list, and focus aligned.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.8.2: Claude and Codex conversations were validated in both directions with real responses confirmed from the correct transcript.',
    ...
    },
    {
      ...
      items: [
        ...
        'Deep Space Light now has consistent contrast across panels, nodes, text, buttons, icons, provider marks, and hover states.',
    ...
    },
    {
      ...
      items: [
        ...
        'The Deep Space bridge provisions Devin\'s project MCP configuration and skill, while ATIF transcripts provide clean agent replies and spoken responses.',
    ...
    },
    {
      ...
      items: [
        ...
        'Deep Space Contributing combines a Claude lead, Codex and Kimi oracles, Svelar/desktop/QA specialists, and a Flow that requires consensus before task creation.',
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.4 is the first macOS release signed with Developer ID Application and notarized by Apple; ad-hoc signing is restricted to local builds.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.3 fixes the 0.1.2 macOS package: the files were intact, but a partial ad-hoc signature made Gatekeeper report that the app was damaged.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.2: the Usage panel now refreshes Claude, Codex, and Kimi automatically every 5 minutes instead of every 60 seconds, reducing unnecessary calls and the risk of HTTP 429 responses.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.1 includes electron-updater in the installed application; Settings no longer mistakes a missing module for running outside the desktop app.',
    ...
    },
    {
      date: 'Aug 07, 2026',
      items: [
        'Deep Space 0.1.0: first public release prepared to update existing 0.0.1 installations.',
    ...
    },
    {
      ...
      items: [
        ...
        'The global deepspace mcp handshake now starts even outside a workspace; token and URL are required only when a tool actually accesses the bridge.',
        'Safe port stopping with confirmation, PID revalidation and Deep Space process protection; arbitrary machine ports are never listed.',
    ...
    },
    {
      ...
      items: [
        ...
        'Fix: the Deep Space MCP server spoke the wrong framing (LSP) and Kimi timed out after 30s — now it is NDJSON, the official MCP stdio standard (Claude, Kimi and co. connect).',
        ...
        'deepspace list now marks who leads with [LIDER] — agents no longer guess "deepspace ask Maestro" (Maestro is the role, not a title).',
      ],
    },
    {
      date: 'Aug 04, 2026',
      items: [
        ...
        'No port fights between workspaces: deepspace port returns a free port and agents learn to never kill a process on someone else\'s port.',
    ...
    },
    {
      date: 'Aug 01, 2026',
      items: [
        'Deep Space is born: agent canvas, CLI bridge, floors (worktrees), routines, roles, kanban, portal and Maestro Mode.',
