...
export const DOCS_PT: DocsCatalog = {
  quickstart: [
    ...
    'Fale com qualquer agente pelo próprio terminal dele, ou deixe o líder distribuir tudo sozinho via CLI deepspace.',
  ],
  sections: [
    {
      id: 'workspaces',
      title: 'Workspaces',
      body: `Um workspace = uma equipe num projeto: diretório de trabalho, ícone e layout do canvas salvos. Crie com o botão + na barra lateral. Vários workspaces rodam ao mesmo tempo — os agentes continuam vivos em background ao trocar. Instruções em AGENTS.md/CLAUDE.md são injetadas nos agentes (edite no lápis ao lado do nome). O botão ⏻ (Descarregar) encerra os terminais vivos e mantém o workspace pausado entre navegações e reinícios do app, sem apagar nada. Abra esse workspace explicitamente para retomar agentes e conversas. No macOS, projetos em Downloads, Documentos ou Mesa exigem consentimento do sistema; se o acesso expirar, o Canvas e o Workbench mostram Autorizar pasta para selecionar novamente o mesmo diretório e continuar sem reiniciar o app.`,
    },
    {
    ...
    },
    {
      ...
      body: `No Windows, o ambiente escolhido ao criar ou editar o workspace é o padrão do time. Cada terminal pode herdá-lo ou, no diálogo de criação e no menu compacto do terminal, escolher Ambiente de execução para forçar Windows nativo ou uma distribuição WSL específica. Selecione exatamente Ubuntu, Ubuntu-22.04, Ubuntu-24.04, Debian ou outra instalação e informe o caminho Linux da mesma pasta do projeto. Assim, um único workspace pode combinar agentes Windows e WSL, inclusive distribuições diferentes. O badge WIN ou WSL identifica uma sobrescrita. Detecção e modelos do provider, PTY, retomada exata da conversa, Council, agentes recrutados e a ponte deepspace seguem o runtime efetivo de cada terminal. Recrutas do Maestro herdam o andar ativo do líder e só são confirmados depois que a PTY inicia no ambiente correto; uma falha remove o nó incompleto. Ao atribuir uma tarefa, o Deep Space inicia ou retoma um agente offline e só move o cartão para Fazendo depois de entregar o briefing completo. Perguntas automáticas, handoffs, atribuições, roles, rotinas e feedback de revisão aguardam o compositor do TUI estabilizar; no Windows e WSL, a entrega só é confirmada quando o prompt exato aparece no transcript do provider, com novas tentativas limitadas apenas do Enter quando necessário. O Deep Space valida a CLI naquela distribuição e lê o transcript do provider na home Linux correspondente antes de persistir ou restaurar um id; um agente vazio começa limpo em vez de adivinhar a conversa mais recente. Trocar o runtime reinicia somente aquele terminal. Distribuição, diretório ou comando ausente gera erros distintos e acionáveis, sem fallback silencioso para Windows nativo.`,
    },
    {
      id: 'wsl-bridge-console',
      title: 'Comunicação confiável dos agentes pelo WSL',
      body: `O pacote Windows inclui um runtime Node de console pinado e verificado por SHA-256. Cada launcher WSL usa esse runtime conhecido quando disponível e mantém o Node da distribuição exata como fallback; nunca chama o Deep Space.exe gráfico para comandos da ponte, pois processos GUI do Windows não devolvem stdout, stderr ou JSON-RPC de forma confiável ao WSL. Workspaces existentes reparam esse launcher automaticamente quando são abertos. Mensagens ligadas a trabalho carregam o id da tarefa Kanban, expiram enquanto aguardam uma conversa anterior e são canceladas antes do composer quando a tarefa termina, é arquivada ou muda de responsável. Um status atrasado não reabre trabalho concluído. Se um ask não tiver confirmação estruturada, recarregue o terminal afetado e tente novamente somente depois de conferir o estado atual da tarefa.`,
    },
    {
      id: 'agentes',
    ...
    },
    {
      ...
      body: `Abra Memória do workspace pelo Command/Ctrl+K, Canvas ou explorador do workspace no Workbench. Salve apenas decisões, fatos, preferências, restrições, referências e aprendizados duráveis; toda entrada exige uma ou mais fontes explícitas, como declaração do usuário, nota, tarefa, mensagem canônica, arquivo do workspace, URL, evidência Git, revisão, Conselho ou agente. Fontes guardam identificação, trechos limitados, links e fingerprints de conteúdo. Revisões nunca sobrescrevem o conhecimento anterior: elas o substituem com proteção de concorrência otimista, e o arquivamento mantém a trilha de auditoria. Agentes usam memory_search apenas quando o contexto for relevante e memory_add ou memory_revise com evidência; o Deep Space nunca injeta toda a memória em cada prompt nem trata conversa casual como fato.`,
    },
    {
    ...
    },
    {
      ...
      body: `Abra a Central de revisão em cada workspace do explorer do Workbench para inspecionar alterações preparadas e não preparadas sem sair do Deep Space. O cabeçalho mostra branch, upstream e commits à frente ou atrás; a lista permite preparar, remover da preparação, criar commit, fazer pull, push e descartar edições rastreadas somente após confirmação. Selecionar um arquivo abre um diff lado a lado e limitado no Monaco; arquivos binários ou muito grandes recebem estados seguros explícitos. Inicie uma revisão para vincular a revisão Git exata a uma tarefa, agente responsável, resumo, evidências, testes, riscos e arquivos selecionados. Clique em qualquer lado do diff para comentar no arquivo ou na linha. Quando o repositório muda, comentários antigos continuam visíveis como contexto desatualizado em vez de apontarem silenciosamente para outra linha. Aprove, solicite alterações ou rejeite em linguagem direta; alterações solicitadas são enviadas ao agente responsável quando seu terminal está disponível e permanecem salvas quando ele está offline.`,
    },
    {
      id: 'git-workspace',
      title: 'Workspace Git nativo',
      body: `Adicione Git pela barra do Canvas ou abra o mesmo node no Workbench. Mudanças prepara e remove arquivos da preparação, cria commits, guarda trabalho no stash e abre o diff exato na Central de revisão. Grafo mostra um histórico limitado de commits com ações de cherry-pick e revert. Branches troca e cria branches, faz merge ou rebase da branch atual e gerencia tags. Worktrees lista o mesmo trabalho isolado dos Andares; crie, revise e aterrisse worktrees por Andares para manter a autoria da tarefa e do agente rastreável. Fetch remove referências obsoletas, pull aceita apenas fast-forward e toda operação roda sem shell. Operações destrutivas mostram o comando exato, exigem confirmação explícita e recusam executar se o repositório mudou depois da prévia. Credenciais de remotes são ocultadas. Agentes usam git_status, git_preview e git_execute sobre esse mesmo estado; a execução exige uma tarefa Kanban ativa e atribuída e fica registrada na Central de controle. Workspaces WSL executam Git dentro da distribuição selecionada, enquanto workspaces Windows nativos continuam usando o Git do Windows. Na titlebar do Windows, o Canvas e o Workbench mostram a branch atual do workspace ativo sem precisar abrir o node — clique nela para o mesmo painel Git num popover rápido.`,
    },
    {
...
console.log('Usuário criado:', body.id);`,
  ...
  expect(res.getBody()).to.have.property('id');
        ...
        {
          id: 'deepspace-native',
          title: 'Deep Space nativo: assertions e JavaScript',
...
Header: Authorization = Bearer {{accessToken}}`,
    ...
    },
    {
      ...
      body: `Notas são markdown vivo compartilhado com os agentes. Uma nota criada por um agente pela ponte se conecta por padrão somente àquele autor exato, então agentes paralelos e roles repetidas mantêm contextos independentes; o agente precisa escolher explicitamente --connect <agente> ou --connect all para compartilhá-la além disso. A convenção: conecte a nota a quem deve lê-la/escrevê-la e diga o propósito no título e no conteúdo. Ex.: nota “Backlog (líder escreve)” conectada ao líder — você escreve “quebre em tarefas para o time” e ele lê com deepspace note read e distribui no quadro. Nota “Para mim (humano)” — peça ao líder para registrar status/decisões nela com deepspace note write/edit, e você acompanha formatado (ícone de olho). Duplo-clique no título renomeia a nota. Solte, cole ou selecione imagens, PDFs, outros arquivos e links HTTP/HTTPS; arquivos de até 10 MB ficam em .deepspace/attachments/ e a referência markdown entra no cursor. Ao remover um anexo pelo X, a referência também sai da nota e o arquivo armazenado no workspace é apagado.`,
    },
    {
      id: 'tarefas',
      title: 'Tarefas (kanban)',
      body: `O nó Tarefas (+ Tarefas na barra inferior) é o quadro do workspace. Use o ícone de colunas no cabeçalho para nomear, colorir, ordenar e criar até dez etapas que representem o seu processo — por exemplo Ideias, Roteiro, Design, Aprovação e Publicado. O líder e o time enxergam essas etapas automaticamente e mantêm o estado real de cada entrega. O botão "Adicionar tarefa" abre um composer completo com título, descrição em markdown e imagens, PDFs, arquivos ou links; também é possível soltá-los direto num cartão. Atribuir um cartão despacha título, descrição e todas as referências direto para o agente. Ao iniciar, o líder recebe todas as tarefas sem responsável com o briefing completo e deve registrar/atribuir no quadro antes de delegar por mensagem. task done envia uma notificação identificada como Tarefa concluída e entrega automaticamente o handoff ao líder assim que o composer dele estiver livre; Projeto concluído é reservado para o término real do projeto. Cada tarefa pode ter UMA nota vinculada; concluídas podem ser arquivadas sem perder o histórico. O botão de expandir em cada cartão abre um detalhe amplo — título completo, descrição em markdown renderizada com checklist clicável, imagens em tamanho legível, anexos, e edição direta de responsável, coluna e nota vinculada — com Anterior/Próximo para navegar entre os cartões da mesma coluna sem fechar.`,
    },
    {
      id: 'imagens',
      title: 'Nó de Imagem (referência visual)',
      body: `A ferramenta Imagem (barra inferior) cria um nó de referência visual no canvas: mockup, screenshot, diagrama de arquitetura. Cole com Ctrl+V ou clique para escolher o arquivo — a imagem fica salva no workspace (.deepspace/images/). Conecte o nó ao líder (ou a um agente específico, como o designer) para deixar claro quem deve usar aquela referência, e diga no chat o que fazer com ela. Duplo-clique no título renomeia; o ícone de imagem no cabeçalho troca o arquivo.`,
    },
    {
      id: 'image-workflows',
      title: 'Fluxos nativos de geração de imagem',
      body: `Abra o menu Imagem na barra do Canvas e escolha Gerar imagens. Conecte Notas para direção de arte, até cinco referências ordenadas de Imagem e um Codex ativo como executor. É necessária uma conta ou assinatura do Codex com ImageGen; nenhuma chave de API da OpenAI é necessária. Digite o prompt, escolha de um a dez outputs, opcionalmente exija PNG realmente transparente e escolha Automático, Instagram quadrado (1080x1080), Instagram retrato (1080x1350), Instagram Stories/Reels ou TikTok (1080x1920), ou um tamanho personalizado de entrega. As mesmas configurações ficam disponíveis ao Codex conectado pelas tools image_workflow_*. Gerar delega um único run lógico à sessão. Cada output é validado separadamente. Quando alpha real é obrigatório e o ImageGen devolve PNG opaco ou quadriculado desenhado, o Codex edita aquele resultado com o ImageGen nativo e tenta até três vezes. Para tamanho exato, o prompt recebe o enquadramento e uma área segura mensurável. A validação preserva o resultado nativo intacto em .masters/ e redimensiona o quadro completo sem recorte somente quando a proporção nativa já corresponde dentro de uma tolerância estrita. Se não corresponder, o validador devolve um prompt nativo de recomposição para o ImageGen preservar o conteúdo, recompor ou expandir o quadro com segurança e passar novamente pela validação antes de materializar o node de Imagem. O Deep Space aceita somente o Codex atribuído e os destinos pré-alocados no workspace, nunca pede nem armazena chave de API de imagem e nunca chama diretamente um endpoint do provider. Os nodes de Imagem registram hash dos inputs, fluxo de origem, id da execução, índice, dimensões exatas, proveniência do master nativo, duração e histórico limitado. Outputs podem alimentar outro fluxo como referências. Cancelar invalida o run ativo para impedir uma conclusão atrasada; execuções abandonadas também expiram. Canvas, Workbench, CLI e MCP operam o mesmo grafo persistido sem estado paralelo de automação.`,
      bullets: [
        'Todo terminal Codex iniciado pelo Canvas recebe o launcher MCP da instalação atual do Deep Space apenas naquela sessão. Uma configuração global antiga do Codex não esconde as tools image_workflow_*, e o arquivo de configuração do usuário nunca é reescrito.',
        'A remoção do fundo e a recomposição de proporção incompatível são sempre edições nativas de image_gen.imagegen usando somente o output rejeitado como referência e o prompt dedicado do validador. Python, Pillow, ImageMagick, ffmpeg, remove-bg, máscaras geradas e processamento de pixels pelo agente são proibidos. O tamanho exato é uma etapa limitada da validação do Deep Space, nunca recorta conteúdo incompatível e sempre preserva o master nativo.',
      ],
    },
    {
    ...
    },
    {
      ...
      bullets: [
        ...
        'Use deepspace graph evidence import <projectId> <caminhoRelativo> --kind auto no terminal do agente, ou a tool MCP code_graph_evidence_import equivalente.',
  ...
  useCases: [
    ...
    },
    {
      ...
      body: 'Ative Executar o Deep Space em segundo plano em Configurações → Autonomia e Core 24/7, configure o terminal vigia como Persistente em Runtime do agente e defina limites de concorrência e uso do provider. Depois crie uma rotina a cada 1–5 minutos no líder: “verifique o quadro (deepspace task list); atribua o que estiver sem dono; se faltar agente, recrute”. Fechar a janela mantém o processo supervisionado e as execuções duráveis na bandeja; Sair do Deep Space interrompe tudo explicitamente.',
      tags: ['Core 24/7', 'Rotinas', 'task assign'],
    },
    {
      id: 'managed-browser-24-7',
      title: 'Execute uma tarefa autenticada no navegador com o Canvas fechado',
      body: 'Adicione e nomeie um Portal, abra Navegador gerenciado, mantenha um perfil do workspace e permita somente os hosts necessários. Faça login você mesmo no Portal visível e crie uma automação agendada de Navegador gerenciado. Agentes usam referências de portal_snapshot e ações tipadas, nunca seletores ou senhas; o Core preserva o perfil, confina arquivos e registra cada efeito no Control Center com a janela fechada.',
    ...
    },
    {
      ...
      title: 'Receba um relatório autônomo por e-mail ou chat',
      body: 'Abra Automações -> Integrações e adicione somente a conta necessária. Conceda operações de leitura para a origem e uma operação explícita de envio para o destino, depois vincule a conta a uma política de autonomia com limites. Crie um agente Persistente ou Sob demanda e uma Automação agendada que reúna os dados autorizados, grave o relatório ou PDF dentro do workspace e chame integration_execute com uma chave de idempotência estável. Gmail e Telegram anexam arquivos do workspace diretamente; Slack envia mensagens para canais e WhatsApp envia mensagens ou um link HTTPS aprovado para o documento. O histórico e a auditoria encadeada guardam ator, operação, destino, status e metadados seguros sem persistir credenciais nem o conteúdo completo da mensagem.',
      tags: ['Central de integrações', 'relatórios agendados', 'Cofre criptografado'],
    },
    {
      id: 'visible-browser-worker',
      title: "Acompanhar um agente no navegador já autenticado",
      body: "Crie um Portal com nome, abra o site desejado e faça login manualmente. Na engrenagem conceda Ler e interagir a um agente e mantenha o segundo plano desabilitado. Em Automações > Segurança ative o modo Delimitado, autorize o host e as categorias de risco necessárias, mantendo envios e ações destrutivas sob gates. Atribua uma tarefa para ler a página, preencher um campo não sensível e preparar um relatório sem enviá-lo. Acompanhe as mudanças na mesma página. Confira o envio pendente em Segurança > Gates, aprove e retome a mesma execução da ferramenta ou aguarde a automação. Expanda Auditoria e Oficina > Execuções para conferir as etapas concluídas. Pause o Portal para testar a revogação. Habilite o segundo plano apenas quando desejar, usando auditoria e histórico para acompanhar.",
      tags: ['Portal', 'Security', 'Tool Workshop'],
    },
    {
      id: 'workspace-tool-workshop',
      title: 'Transforme uma operação repetida numa ferramenta reutilizável',
      body: 'Adicione Tool Workshop e crie um rascunho com contrato tipado de entrada e saída, timeout, limite de saída e pelo menos uma fixture. Use uma integração conectada para Gmail, Slack, Telegram, WhatsApp ou GitHub; HTTP para uma API aprovada e limitada; Transformação para organizar dados deterministicamente; ou Comando do workspace para um executável confinado com argumentos explícitos. Guarde credenciais em Segurança como SecretRefs vinculadas ao Tool Workshop, a tool:<slug> e ao destino exato. Rode o teste seco, confira manifesto e revisão e então publique pessoalmente. Agentes com tarefas ativas atribuídas podem propor revisões posteriores e executar a publicada, enquanto automações chamam a mesma ferramenta com idempotência estável. Um rascunho proposto não substitui silenciosamente o comportamento em produção, e histórico mais Auditoria mostram quem chamou qual revisão sem reter segredos.',
      tags: ['Tool Workshop', 'ferramentas versionadas', 'SecretRef'],
    },
    {
      id: 'desktop-app-automation',
      title: 'Automatize um aplicativo instalado no computador',
      body: "Peça ao agente: \"Abra a Calculadora, calcule 73 vezes 19 e confira o resultado na tela.\" Ele cria a nota, a tarefa atribuída e o nó Computador, reutiliza o aplicativo autorizado, captura a evidência de 1387 e registra o resultado. Você concede apenas acesso do sistema e o limite de workspace/aplicativo, não cada clique comum. Remotamente, faça o mesmo pedido pela Equipe ou conversa com o líder. Para consultar um e-mail já aberto no Chrome do desktop, peça explicitamente para reutilizar aquela janela e conta. Enviar e-mail exige o risco declarado external_publication e o gate configurado; desafios de login ficam com o usuário. O desktop host precisa estar disponível. Confira nota, tarefa, captura e auditoria reais, não somente uma mensagem de sucesso.",
      tags: ['Nó Computador', 'automação desktop', 'apps permitidos'],
    },
    {
    ...
    },
    {
      ...
      body: 'Rotina de hora em hora num shell ou agente: “rode os testes; se falhar, abra uma tarefa para o time e me notifique (deepspace notify)”. Você recebe notificação nativa do sistema e o kanban já tem o cartão.',
    ...
    },
    {
      ...
      body: 'Abra Roles, escolha "Descobrir em outra pasta..." e selecione o projeto que possui a role. O Deep Space valida os arquivos limitados em .deepspace/roles desse projeto, importa apenas nomes novos e nunca sobrescreve uma role existente no workspace.',
    ...
    },
    {
      ...
      title: 'Contribuir no Deep Space com três perspectivas',
      body: 'Aplique Deep Space Contributing. O Claude lidera, Codex e Kimi atuam como oráculos independentes e precisam aprovar o plano antes de qualquer tarefa ser criada; especialistas Svelar, desktop e QA/release executam o plano já documentado.',
    ...
    },
    {
      ...
      body: 'Abra Design na barra do Canvas e escolha Três direções completas de UI. Informe objetivo, público, plataforma, destino do código, restrições, referências e se Light e Dark são obrigatórios. O Deep Space cria um grupo rastreável com uma spec vinculada, oito tarefas progressivas no Kanban e três documentos Design nativos: Clareza, Expressiva e Eficiente. Cada designer começa somente com uma tela principal desktop e uma mobile, preferencialmente por importação semântica HTML/CSS, e deve mostrar a primeira revisão em até cinco minutos. O status no node distingue aguardando, trabalhando, parado e pronto para revisão. Abra o documento, use a seção Revisão visual de Qualidade para aprovar ou devolver com feedback; contagem de layers e auditoria estrutural não substituem essa inspeção. Somente a direção aprovada avança para estados responsivos, tokens tipados, componentes, protótipo e preview de código real. Em documentos largos, navegue com trackpad, Mão (H), Espaço+arraste ou botão do meio e use Ajustar. Finalize validando o resultado aprovado contra um Portal ou dispositivo móvel e registrando-o no Review Center.',
      tags: ['3 direções de UI', 'design + tokens + código', 'aprovação humana'],
    },
    {
      id: 'design-precision',
      title: 'Selecione, meça e organize arte detalhada',
      body: 'Abra o documento Design nativo em seu node do Canvas ou no modo foco. Passe o cursor sobre uma camada para prever seu limite exato, clique para selecionar o grupo ou use Command/Ctrl+clique repetidamente para entrar na arte aninhada e alternar camadas sobrepostas. Segure Alt/Option ao apontar outra camada para ver distâncias horizontais e verticais exatas. Gire pela alça direta e segure Shift para encaixar em 15 graus; arraste contra camadas ou guias para ver coordenadas inteligentes rotuladas. Selecione várias camadas para alinhar, distribuir ou usar Organizar, que preserva as linhas aproximadas e move cada hierarquia completa de descendentes. Agentes conectados usam design_arrange_elements e design_edit_vector, ou os comandos equivalentes da CLI incluída, sobre o mesmo documento revisionado.',
      tags: ['seleção profunda', 'medidas + rotação', 'tools tipadas de agentes'],
    },
    {
      id: 'design-figma',
      title: 'Desenhe uma interface junto com seu time de IA',
      body: 'Adicione um node Design nativo e abra no Modo Design do Canvas ou no Workbench. Monte paths vetoriais, máscaras, gradientes, frames responsivos com auto layout e assets de imagem reutilizáveis ou cole um SVG para transformar paths, transforms, estilos e gradientes em camadas nativas editáveis. Agrupe ou desagrupe a arte, selecione todas as camadas que usam a mesma cor, substitua cores correspondentes em sólidos e stops de gradiente e copie a seleção como SVG ou PNG. Em Variáveis, use presets, importe ou exporte tokens, defina modos e aliases, vincule propriedades e audite repetições. Em Componentes, crie fontes, instâncias, propriedades, variantes, slots e overrides. Publique bibliotecas versionadas apenas para workspaces autorizados ou extraia e sincronize CSS variables, Tailwind e contratos Svelte, React ou Vue pela varredura estática da aba Código. Em Inspecionar → Figma, mantenha o MCP remoto oficial gerenciado para agentes compatíveis, salve um token REST somente leitura no cofre do sistema operacional, inspecione o link do arquivo, escolha páginas ou frames e importe camadas, vetores, assets, estilos, variáveis, componentes, variantes, instâncias locais e identidades de bibliotecas externas para o mesmo documento nativo. Origens vinculadas comparam hashes remotos e locais antes de uma sincronização seletiva, para você decidir entre alteração do Figma, edição local e conflito. Escolher a versão local coloca somente aquela layer revisada na fila do Figma. O plugin próprio transfere a seleção ativa do Figma com imagens raster, copia SVG editável ou JSON estrutural, cria uma página no Figma com assets, variáveis, estilos, componentes e variantes nativos do documento Deep Space e envia somente as alterações vinculadas da fila de volta ao arquivo atual por uma conexão do workspace restrita ao loopback. Conecte o documento ao Designer ou líder: o agente lê a revisão exata, combina o MCP oficial do Figma com tools tipadas de inspeção, import e sync do Deep Space e verifica o resultado enquanto seu editor atualiza ao vivo. Os mappings existentes de Code Connect completam o vínculo persistente node Figma → camada Deep Space → implementação. Exporte a seleção ou a página aprovada em SVG, PNG, JPEG, WebP ou PDF. Documento, assets, thumbnails, design system, vínculos Figma e histórico de revisões ficam no workspace e continuam pesquisáveis junto com tarefas, notas, arquivos, portais e o restante do time.',
      tags: ['Modo Design nativo', 'vetores + auto layout', 'manual + agentes'],
    },
    {
      id: 'design-delivery',
      title: 'Transformar design em código e validar a implementação',
      body: 'Abra um documento Design nativo e escolha Inspecionar → Código. Importe estruturas HTML/Tailwind, Svelte, React/JSX ou Vue como layers nativas editáveis sem executar código do projeto. Para entregar, selecione um frame ou grupo, escolha o adapter Svelar/Svelte 5, React, Next.js, Vue 3 ou HTML/Tailwind, revise o arquivo gerado completo e somente então grave dentro do workspace. Mappings compatíveis de Code Connect reutilizam primeiro os componentes reais do projeto; o artefato vinculado abre direto no Monaco e se recusa a sobrescrever um arquivo alterado depois da prévia. Agentes conectados usam design_import_code e design_generate_code_preview/apply pelo MCP tipado do Deep Space, ou os comandos equivalentes da CLI incluída, com a mesma revisão e atribuição à tarefa. Explorações guiadas mostram um checklist de oito partes no node Design e no painel Qualidade: frames exigidos pela plataforma, Branding board visível, sistema de tokens tipados, bindings em layers reais, componentes, protótipo, artefato de código aplicado e aprovação visual da revisão atual. Aprovar o conceito escolhe um único alvo de entrega e, quando o líder executa o fluxo, despacha sua tarefa rastreada de expansão; o Kanban impede concluir expansão, implementação ou validação final enquanto faltar evidência. Em Validar, escolha um Portal ao vivo ou dispositivo iOS/Android conectado e um viewport de frame, celular, tablet ou desktop. O Deep Space captura a implementação, normaliza as duas imagens e mostra design, implementação, overlay ajustável e pixel diff. Crie uma tarefa de feedback no Kanban com os três screenshots ou uma entrada na Central de Review vinculada à alteração Git real para que líder ou especialista possa reproduzir, atribuir e aprovar o resultado.',
    ...
    },
    {
      ...
      body: 'No cabeçalho do agente, abra ⇄ e escolha outro provider instalado. O Deep Space encerra somente a PTY e a conversa do provider anterior, preserva nome, role, Modo Maestro, andar, posição e conexões e inicia o substituto no mesmo nó.',
      tags: ['Providers', 'troca sem recriar', 'time preservado'],
    },
    {
      id: 'devin-local-agent',
      title: 'Usar o Devin como membro local do time',
      body: 'Instale e autentique a CLI oficial do Devin e crie um agente Devin pelo canvas. Pesquise a lista de modelos da conta no seletor limitado e rolável, escolha um e inicie com acesso autônomo ao workspace. O Deep Space provisiona a ponte MCP e a skill nativas e retoma a conversa local exata depois de reiniciar o app.',
    ...
    },
    {
      ...
      body: 'Capture o workspace atual como Team Pack customizado, publique uma versão semântica com notas e inspecione checksums imutáveis. Exporte para outra instalação ou importe um arquivo compartilhado após o Deep Space validar schema, tamanho, conteúdo e SHA-256. Agentes, roles, skills, etapas, rotinas, configuração MCP e layout viajam; sessões e credenciais não.',
    ...
    },
    {
      ...
      body: 'Arraste uma imagem, PDF, arquivo ou link HTTP/HTTPS para a nota de briefing, o composer de um agente ou um cartão do kanban. O Deep Space guarda arquivos de até 10 MB dentro do workspace, insere uma referência legível e entrega título, descrição e todos os anexos quando o líder ou agente recebe a tarefa.',
    ...
    },
    {
      ...
      title: 'Gerenciar branches e commits com pessoas e agentes',
      body: 'Adicione Git ao Canvas e inspecione Mudanças, Grafo, Branches e Worktrees. Prepare os arquivos corretos, faça commit, troque ou crie uma branch e revise merge, rebase, cherry-pick, revert, tag, stash, fetch, pull ou push antes de executar. Use Andares para worktrees isolados e a Central de revisão para o diff e a decisão final. Um agente atribuído usa as mesmas operações protegidas pelo MCP ou CLI com o id da tarefa Kanban; ações destrutivas continuam exigindo confirmação e uma revisão atual do repositório.',
      tags: ['Cliente Git nativo', 'branches e worktrees', 'operações de agentes'],
    },
    {
    ...
    },
    {
      ...
      body: 'Habilite o compartilhamento experimental, inicie uma sessão criptografada de ponta a ponta e escolha um convite para Navegador/celular ou App Deep Space. O link web abre o PWA Remote instalável. O convite do app abre automaticamente o Deep Space instalado; o convidado também pode usar Workspace → Entrar em workspace remoto e colar o convite. Ambos removem o segredo da URL antes da conexão e guardam uma chave de pareamento não extraível somente naquele dispositivo. Confira a impressão digital e escolha Leitor, Colaborador, Operador ou Administrador. Um Operador pode manter conversas sanitizadas e rastreáveis com o líder ou outro agente e ditar em qualquer uma delas usando o STT local do host. A tela principal mantém o histórico do líder visível; quando ele usa ferramentas e fala em várias etapas, o Remote aguarda o fim real do turno e reúne todos os blocos antes de publicar a resposta. Abra Equipe para inspecionar o foco atual, a atividade semântica recente e os totais limitados de coordenação de cada agente sem expor mensagens internas nem a saída do terminal. No celular, a navegação fixa mantém Visão geral, Equipe, Tarefas e Atividade disponíveis; Mais abre Huddles e Revisões, as colunas viram uma visualização vertical legível e toda tela de detalhe tem uma ação explícita de retorno. Um Administrador também pode iniciar ou restaurar um agente offline. O terminal bruto é uma permissão separada, exclusiva de Administrador naquele dispositivo, desativada por padrão, limitada a um terminal, responsiva à tela atual, com limite de tráfego, criptografia e auditoria. O ditado do terminal insere o texto sem pressionar Enter. Ela não libera navegação de arquivos, visualização de Portal ou dispositivo mobile nem edição do Canvas. Revogue o dispositivo ou encerre a sessão a qualquer momento e consulte comandos aceitos e rejeitados na auditoria.',
    ...
    },
    {
      ...
      body: 'Abra o menu de opções de um terminal e escolha Comandos salvos. Guarde atalhos exclusivos daquele terminal ou comandos globais disponíveis em todos eles, pesquise pelo nome ou conteúdo e execute qualquer item manualmente. Em shells puros, ative Executar ao retomar para disparar os comandos uma única vez quando a sessão for criada ou restaurada, inclusive no WSL. O Deep Space nunca autoexecuta texto em Claude, Codex, Kimi ou outro agente para não contaminar conversas. Os comandos ficam em texto simples: use variáveis de ambiente ou o cofre da ferramenta para segredos, nunca senhas e tokens no comando salvo. Os terminais preservam o ambiente do sistema operacional e a ponte do Deep Space, mas excluem valores privados do servidor desktop para que o .env de cada projeto, incluindo a APP_KEY do Laravel, tenha prioridade.',
      tags: ['Comandos salvos', 'autoexec seguro', 'shells e WSL'],
    },
    {
      id: 'creative-image-workflow',
      title: 'Crie um personagem, aplique a marca e entregue um carrossel',
      body: 'Inicie o caso de uso guiado para validar a cadeia completa sem montar o Canvas manualmente. A cada avanço, o Deep Space cria um Diretor Criativo Codex, briefings reutilizáveis de personagem e campanha, um logo PNG de teste e três workflows sequenciais: Personagem Master, Personagem com Marca e Carrossel XYZ. Fazer por mim dispara cada geração real; o tour só libera a próxima etapa depois de validar os arquivos no workspace e materializar os nodes Imagem. O primeiro resultado de uma etapa é conectado automaticamente como referência ordenada da seguinte, preservando prompts, caminhos, procedência e histórico. O exemplo gera três masters, duas poses com marca e três slides para uma validação rápida; cada workflow aceita até dez outputs. É necessária uma conta ou assinatura Codex autenticada com ImageGen, sem chave de API da OpenAI.',
      tags: ['Codex ImageGen', 'personagem e marca', 'carrossel guiado'],
    },
    {
      id: 'code-intelligence-graph',
      title: 'Entender uma codebase antes de alterá-la',
      body: 'Inicie o caso de uso guiado para adicionar um nó Grafo de Código e indexar os repositórios aprovados. Mantenha o modo Assistido recomendado para que edições estabilizadas disparem atualização incremental e cada consulta de agente espere a revisão conhecida mais recente; escolha Manual para indexação explícita apenas pelo usuário ou Desativado para parar watchers e bloquear o grafo sem apagar seu histórico. Pesquise um serviço, componente, rota ou método, explique uma relação com sua procedência e abra o símbolo exato no Monaco. Sobreponha agentes, tarefas e Floors ativos para expor conflitos de propriedade e monte um pacote de contexto revisável dentro do orçamento escolhido para o líder, um agente, o Council ou uma tarefa rastreável. Compare revisões e salve projeto, visão, filtros, seleção, câmera e código aberto como uma investigação nomeada. Contratos, Qualidade, Busca semântica, Execução e Impacto das alterações continuam no mesmo grafo, e os agentes usam essas operações limitadas pelo MCP sem carregar repositórios inteiros na conversa.',
      tags: ['Inteligência de código', 'contexto operacional', 'múltiplos repositórios'],
    },
    {
    ...
    },
    {
      ...
      body: 'Passe o mouse perto de qualquer nó do canvas e uma bolinha de conexão aparece em cada um dos quatro lados — cima, direita, baixo e esquerda — em vez de só na borda direita. Arraste a partir do lado mais próximo do nó que você quer alcançar, então ligar agentes, notas e ferramentas não exige mais rodear a conexão até sempre o mesmo ponto.',
      tags: ['Canvas', 'conexões', 'bolinhas de conexão'],
    },
    {
      id: 'customizable-toolbar',
      title: 'Fixe só as ferramentas que você usa',
      body: 'O botão no fim da barra inferior do canvas abre o menu "mais ferramentas", listando todo tipo de nó e painel que não está fixado na barra no momento; clique num item pra usá-lo na hora. Abra o submenu Fixados pra marcar ou desmarcar exatamente quais ferramentas ficam visíveis — um mínimo continua fixado pra barra nunca ficar vazia — e a escolha fica salva entre workspaces e reinícios do app.',
      tags: ['Barra de ferramentas', 'personalização', 'itens fixados'],
    },
  ],
  changelog: [
{
  ...
  "items": [
    ...
    "Ligar dois nós não depende mais de acertar um alvo de 20px: o raio de conexão aumentou, então soltar a corda perto do nó de destino conecta. Os pontos de conexão de todos os nós também ficam visíveis enquanto a corda está sendo arrastada — antes o nó de destino não mostrava nada, porque os pontos só apareciam com o mouse em cima e o ponteiro estava preso no arraste.",
    ...
    "O Usage lista um perfil adicionado manualmente logo depois do provedor a que ele pertence, em vez de no fim da lista. O painel de roteamento do líder também perdeu o campo desalinhado: a dica de janela ficava dentro da grade e esticava uma coluna."
  ]
},
{
  ...
  "items": [
    "A barra de ferramentas, os controles de zoom e a barra de ação da seleção saíram da superfície do canvas para uma faixa própria, abaixo dela. Antes moravam na mesma área dos nós, então um nó arrastado por cima os escondia — e um Portal os escondia de vez, porque a página dentro de um Portal é uma camada nativa desenhada acima da janela inteira, na frente da qual nenhuma ordem de empilhamento consegue ficar. Fora daquela área, nada no canvas os alcança. Em troca o canvas perde cerca de 55px de altura. O minimapa continua onde estava, porque só funciona dentro da superfície do canvas; ele ainda pode ser coberto por um Portal, e pode ser desligado em Configurações."
  ]
},
{
  "date": "21 de setembro de 2026 · 0.33.3",
  "title": "Deep Space 0.33.3: Portal para de recarregar sozinho",
  "summary": "Fim da recarga a cada poucos segundos que roubava o foco de quem estava digitando",
  "items": [
    "Um Portal aberto se recarregava sozinho de poucos em poucos segundos e levava o foco do teclado junto, interrompendo o que estava sendo digitado num terminal. Depois de cada carregamento o Portal fazia uma checagem lendo location.href dentro da página e tratava resposta vazia como página quebrada — então recarregava. SPA trocando de rota, redirect em curso ou subframe carregando respondem vazio, e uma checagem bem-sucedida zerava o contador de tentativas, então o ciclo não terminava nunca. Agora só falha real recarrega: a página ser uma página de erro do Chromium, a navegação ser recusada ou o carregamento acusar erro. Checagem que não consegue responder é inconclusiva e não muda nada."
  ]
},
{
...
},
{
  ...
  "items": [
    "Nome do produto, identificador do aplicativo, instalador, atalhos, documentação e textos da interface nos três idiomas passaram a usar Deep Space. A atribuição ao projeto original permanece em NOTICE, LICENSE e no README. A descrição do pacote passou a ser a assinatura do produto, que também alimenta o resumo exibido pelos instaladores.",
    "A atualização automática e a automação de release apontam para o repositório do Deep Space, então uma instalação nunca oferece o aplicativo original por cima dela.",
    "O endereço do relay de compartilhamento passou a ser configurável por PUBLIC_RELAY_URL, com o passo a passo de implantação em docs/relay.md.",
    "A ponte dos agentes também foi renomeada: o comando virou deepspace, a pasta virou .deepspace/, as variáveis de ambiente viraram DEEPSPACE_ e o protocolo virou deepspace:. Ela tinha ficado com o nome antigo para proteger workspaces de instalações anteriores; como nenhuma versão foi publicada, não havia o que proteger. Um workspace criado antes é reparado na primeira abertura.",
    "O tema padrão ficou monocromático sobre quase preto. O destaque é branco, e página, canvas e barra lateral compartilham o mesmo tom, então os painéis deixam de parecer faixas cinzas contra o conteúdo. O texto secundário foi clareado para passar no contraste mínimo, e o tema claro recebeu o equivalente neutro.",
    "A tela de abertura foi redesenhada: preto, com luz fria vindo da superfície, partículas finas subindo e pontos de carregamento em branco gelo. Ela passou a seguir o idioma do sistema, que já está disponível antes das configurações, e usa inglês para qualquer outro idioma.",
    "Os ícones do aplicativo foram trocados em todos os destinos: Windows, Linux, macOS, bandeja, favicon e a marca dentro do app. No Windows e no Linux a arte preenche a caixa de ponta a ponta; no macOS ela mantém a margem que a grade da Apple espera.",
    "O prompt rápido do terminal pode ser recolhido, devolvendo o espaço para quem não anexa arquivos nem menciona agentes com @. Cada terminal lembra o próprio estado, e soltar um anexo reabre o campo.",
    "Grupos de workspace (pastas na barra lateral) agora podem ter um ícone próprio, igual ao seletor que já existia em workspaces e projetos.",
    "Nós do canvas podem ser conectados por qualquer lado — cima, direita, baixo ou esquerda — em vez de só pela borda direita, com bolinhas que aparecem ao passar o mouse.",
    "A barra de ferramentas inferior do canvas agora é personalizável: as oito ferramentas mais comuns ficam fixadas por padrão, e o resto vai pro menu 'mais ferramentas', onde qualquer item pode ser fixado ou desafixado.",
    "O instalador do Windows agora mostra uma imagem lateral personalizada durante a instalação.",
    "Os ícones de compartilhar e notificações saíram da barra lateral do Workbench e foram pra titlebar nativa do Windows; macOS e Linux continuam com eles na barra lateral.",
    "O seletor de abas Canvas/Workbench agora preenche toda a largura do container.",
    "Correções: a tela de abertura não fica mais presa depois que o app carrega e passou a cobrir a subida do servidor; a janela volta a ser arrastável pela área vazia da barra de título; o terminal desenha corretamente os blocos usados nos banners das CLIs; os controles de ditado deixaram de cobrir a linha de prompt; a barra de tarefas do Windows mostra o ícone do app; o link de download manual aponta para as versões deste projeto; os ícones de importar e exportar tema foram destrocados; o log virou deep-space.log, igual ao nome citado na documentação; a lista de perfis de um provider agora carrega já na primeira vez que o card é expandido; a expansão de '~' parou de misturar separador de barra e contrabarra no Windows nativo; o painel de Uso mostra um erro explícito em vez de ficar em branco num rate limit; duplo clique numa palavra ou triplo clique numa linha do terminal volta a selecionar; um terminal maestro parou de reentregar a mesma tarefa a cada reconexão; e um botão de compartilhar duplicado no Windows parou de dobrar o polling do canvas no endpoint de colaboração."
  ]
},
{
  "date": "13 de setembro de 2026 · 0.30.1",
  "title": "Deep Space 0.30.1: um único atalho para colar em qualquer terminal",
  ...
  "items": [
    ...
    "Imagem ou arquivo colado no terminal vira anexo do workspace em .deepspace/attachments/ e o caminho é colado no prompt, o mesmo contrato que o composer do canvas já usa para entregar referências aos agentes.",
    "Alt+V continua livre para as CLIs que documentam esse atalho próprio."
  ]
},
{
  "date": "11 de setembro de 2026 · 0.30.0",
  "title": "Deep Space 0.30.0: trabalho no desktop por linguagem natural e acesso ao microfone",
  "summary": "Peça ao agente para operar a sessão que já está aberta",
  "items": [
    "O app e os helpers assinados no macOS agora incluem permissões de assinatura para microfone e Apple Events. O empacotamento local e a CI conferem as permissões reais, além da assinatura e notarização. O modelo de STT não mudou.",
    "Agentes podem preparar e conectar o nó Computador, criar sua própria tarefa atribuída sem disparar outro prompt para si mesmos e abrir ou reutilizar aplicativos nativos autorizados pela CLI e pelo MCP. Janelas existentes do navegador mantêm a sessão autenticada.",
    "A preparação herda somente uma autorização prévia limitada para Computador e aplicativos, sem reativar um nó pausado pelo usuário. É possível autorizar um ID antes de abrir o app; aplicativos autorizados e fechados continuam visíveis e podem ser revogados.",
    "O Computador usa identificadores nativos estáveis no macOS, recusa janelas obsoletas ou ambíguas, verifica o foco antes de digitar, impede disputa de entrada entre workspaces e bloqueia repetição automática após falhas potencialmente parciais.",
    ...
    "O inventário do Computer unifica registros nativos idênticos e exclui identidades conflitantes. Janelas duplicadas do Finder no macOS não travam mais a renderização nem o indicador de carregamento; falhas permitem tentar novamente e a atualização mantém o painel visível.",
    "A execução de fluxos aguarda o salvamento das etapas no Canvas e Workbench. Falhas ao salvar impedem a execução com etapas antigas ou ausentes.",
    ...
    {
  "date": "10 de setembro de 2026 · 0.29.0",
  "title": "Deep Space 0.29.0: Agentes persistentes, fluxos visíveis no navegador e ferramentas reutilizáveis",
  ...
  "items": [
    ...
    "Fechar todas as janelas mantém o trabalho ativo habilitado na bandeja; Sair do Deep Space continua sendo o comando explícito que encerra o Core e seus processos filhos.",
    ...
    "A Central de integrações agora conecta Gmail, Slack, Telegram, WhatsApp, GitHub e webhooks HTTPS com permissões por operação. OAuth e credenciais de apps ficam no Cofre do sistema operacional, enquanto agentes usam ações tipadas e idempotentes com histórico sanitizado.",
    "Um novo nó Computador inspeciona e controla apps desktop explicitamente permitidos no macOS, Windows e sessões Linux X11 compatíveis, com comandos tipados e idempotentes, evidências de captura limitadas e auditoria sem conteúdo digitado.",
    "O Tool Workshop cria ferramentas reutilizáveis e versionadas do workspace a partir de integrações, HTTP limitado, transformações determinísticas e comandos confinados. Agentes propõem rascunhos, o dono publica revisões imutáveis, automações executam com idempotência e vínculos exatos de SecretRef mantêm credenciais dentro dos executores confiáveis.",
    "Formulários do Workshop mantêm identidades independentes no Canvas e nos painéis, e executores de navegador exibem o nome correto.",
    "O Portal acompanha as mudanças de geometria do Canvas sem polling de 100 ms, ocultação repetida da aba ou reaplicação desnecessária do zoom. O Codex encaminha a identidade efêmera da ponte ao MCP, e prompts rápidos usam a fila de envio confirmado.",
    "Salvar políticas de autonomia, aprovar gates e criar SecretRefs agora valida corretamente os parâmetros das rotas HTTP.",
    "Popups do navegador permanecem dentro do Portal e preservam o vínculo com a página de origem, sem abrir uma janela separada do sistema. Capturas protegidas aguardam um frame novo, inclusive no controle em segundo plano habilitado explicitamente.",
    "Agentes operam a mesma página nativa visível do Portal, com acesso por agente, modos leitura/interação, segundo plano opt-in, pausa imediata, campos protegidos nas capturas, referências antigas rejeitadas e sem scripts arbitrários. Agentes pré-autorizados publicam ferramentas delimitadas de navegador/integração/HTTP/transformação; a retomada de gates preserva checkpoints e revisão. Auditoria detalhada, webhook local e parada de emergência persistente ficam explícitos na UI."
  ]
},
    {
      date: '7 de setembro de 2026 · 0.28.0',
      title: 'Deep Space 0.28.0: entrega social exata sem recorte inseguro',
      ...
      items: [
        ...
        'O ImageGen recebe uma área segura mensurável para o quadro escolhido. O Deep Space preserva o master nativo e redimensiona a imagem completa somente quando a proporção já corresponde; resultados incompatíveis voltam ao ImageGen nativo para recomposição ou expansão segura em vez de recortar conteúdo.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.27.0: o editor de Design ganha cara de ferramenta de design',
    ...
    },
    {
      ...
      title: 'Deep Space 0.26.1: Andares Git confiáveis no WSL',
    ...
    },
    {
      ...
      title: 'Deep Space 0.26.0: workspace profissional de Design nativo',
      summary: 'O mesmo node Design agora se expande em um workspace focado e responsivo sem perder o contexto visual.',
      items: [
        'Um node Git nativo agora gerencia mudanças, histórico de commits, branches, tags, remotes, stashes e worktrees de Andares no Canvas e Workbench. Operações protegidas por CLI/MCP usam a mesma revisão do repositório, exigem tarefa ativa atribuída e o terminal vivo autenticado daquele agente, ocultam credenciais e executam na distribuição WSL selecionada quando aplicável.',
        'O modo focado ocupa todo o viewport do app e retorna ao node exato do Canvas preservando revisão, seleção, ferramenta ativa, zoom, câmera e estado dos painéis.',
        'Conceitos guiados de Design agora exigem aderência literal ao brief do produto, composição legível em 100%, raios moderados e auditoria sem erros; placeholders, labels de debug, decoração arbitrária e pills esticados não podem ser tratados como trabalho concluído.',
        'O modo focado de Design desabilita atalhos de exclusão do Canvas sem registrar um atalho vazio, mantendo o diagnóstico desktop livre do aviso repetido do renderer.',
        'Entrar ou sair do workspace focado de Design libera o foco antes de ocultar a superfície anterior, evitando controles focados inacessíveis e avisos de acessibilidade do Chromium.',
        'A atribuição do Kanban aguarda o terminal de um agente recém-recrutado chegar ao primeiro prompt estável, evitando trabalho marcado em andamento quando o primeiro handoff se perdeu durante a inicialização da CLI.',
        'Duplicar uma camada aninhada de Design agora preserva a hierarquia completa mesmo quando o filho armazenado aparece antes do frame ou grupo pai.',
        'Imports de HTML, Svelte, React e Vue preservam larguras explícitas de desktop e mobile, seletores descendentes com escopo, tipografia herdada, padding completo do box model CSS, controles que abraçam o conteúdo, texto inline misto, comportamento padrão de linha do flex, espaço flexível restante e navegação e ações flutuantes absolutas dentro do frame mobile; o wrapper cresce ao redor da arte real em vez de recortar, sobrepor ou comprimir o conteúdo.',
        'Um agente permanece autenticado no workspace nativo ou WSL original depois de entrar em um diretório scratch ou irmão; o processo carrega somente o caminho da configuração existente, nunca uma cópia do token.',
        'Ferramentas de criação ficam numa barra inferior estável; ações de documento, organização, visualização, protótipo e exportação permanecem na barra superior compacta.',
        'Camadas, assets e propriedades podem ser recolhidos separadamente e viram overlays dispensáveis em superfícies estreitas do Canvas e Workbench.',
        'Command/Ctrl + roda aplica zoom ao redor do ponteiro, e os comandos de visualização enquadram todo o conteúdo, a seleção ou restauram 100%.',
        'Ajustar todo o conteúdo e Ajustar à seleção usam os limites da arte visível em vez de reduzir páginas quase vazias às dimensões nominais.',
        'O workspace infinito segue o tema e fica visualmente separado do fundo da página real, evitando que frames distantes criem uma cena branca gigante.',
        'Páginas podem ser criadas, renomeadas, duplicadas, reordenadas, ativadas e excluídas com segurança; a árvore pesquisável de camadas oferece recolhimento, rename inline, visibilidade, bloqueio, reparent preciso por arraste e ações de contexto.',
        'Copiar, recortar, colar e duplicar com Command/Ctrl preserva hierarquias completas de descendentes, enquanto operações tipadas de página dão aos agentes o mesmo controle protegido por revisão via MCP e CLI.',
        'A seleção adiciona contorno no hover, ciclo em sobreposições aninhadas, rotação direta, medidas com Alt/Option, smart guides rotulados e alinhamento, distribuição e Tidy Up determinísticos pelo mesmo contrato tipado dos agentes.',
        'O inspector compacto adiciona cálculos e scrub aos campos numéricos; fixed, hug, fill, min/max e auto layout completo; tipografia rica com novas camadas de texto que crescem verticalmente; edição multisseleção; e fluxo acessível por teclado para cor e gradiente em Hex/RGB/HSL com alpha, conta-gotas, cores do documento e vínculo com variável.',
        'O painel direito agora usa abas focadas em Design, Protótipo e Inspecionar. As conexões de protótipo ficam visíveis e podem ser desenhadas diretamente no canvas, enquanto Inspecionar reúne acessibilidade, tokens, contratos de componente, Code Connect, origem Figma, artefatos e CSS copiável.',
        'Agentes conectados podem manter fluxos e interações de protótipo por operações MCP e CLI tipadas; revisões de agentes e histórico de qualidade abrem como drawers contextuais sem substituir as propriedades da layer.',
        'Documentos Design aguardam o layout do SVG antes de restaurar a câmera, protótipos grandes ocultam conexões fora da tela sem perder endpoints selecionados e a presença colaborativa permanece válida pelo contrato de rota do Svelar FormRequest.',
        'Diálogos de confirmação abertos pelos drawers contextuais de Design permanecem acima do editor focado e recebem o clique em vez de serem renderizados atrás do canvas.',
        'Pré-visualizações do Design no Canvas são regeneradas em 2048 px, mostram somente nomes de frames de primeiro nível e reabrem com limites justos, sem arte borrada nem região preta vazia.',
        'Explorações guiadas exibem um checklist de oito partes e bloqueiam a conclusão das etapas até a direção aprovada ter Branding board, frames exigidos pela plataforma, tokens tipados e vinculados, componentes, protótipo, artefato de código aplicado e aprovação visual atual.',
        'Aprovar uma direção registra um único alvo de entrega, conclui o gate de revisão e despacha automaticamente a tarefa rastreada de expansão quando o fluxo é executado pelo líder; cada gate concluído coloca implementação e validação na fila em ordem, e qualquer falha de despacho aparece como bloqueio visível. A aprovação fica desabilitada enquanto a tarefa de conceito ainda edita o documento, e uma revisão posterior preserva os metadados de review enquanto exige um novo gate.',
        'O MCP de Design publica o contrato completo das propriedades de componente, preenche a ordem omitida e aceita lotes de blueprint ou operações brutas com no máximo 100 itens protegidos por revisão. Comentários, respostas, resoluções, propostas pendentes e navegação de página versionados preservam um review visual válido porque não mudam a entrega revisada.',
        'Eventos de ciclo de vida da PTY permanecem vinculados à tarefa Kanban ativa. Um terminal retomado ignora o ruído de desconexão para encontrar o último estado semântico da tarefa, resolve a atenção obsoleta e redespacha trabalho bloqueado ainda ativo depois que a TUI fica pronta; o startup mantém essa recuperação na fila até o quadro estar disponível. O líder precisa rever a causa, reconciliar goals do provider, Central de controle e Kanban, executar a validação e só então anunciar conclusão.',
        'Uma direção conceitual agora tem cinco minutos para o primeiro gate completo, não apenas para a primeira revisão: uma importação semântica, no máximo uma correção, uma auditoria e então revisão visual humana antes de expandir branding, tokens, componentes, protótipo ou código.',
        'Agentes Codex com acesso total confiam somente no workspace nativo ou WSL exato durante aquele processo, impedindo que o despacho de tarefas seja consumido pela tela inicial de confiança do diretório; a configuração global do Codex e a confirmação do acesso padrão permanecem intactas.',
        'A criação de uma exploração Design executada pelo líder é atômica: uma falha no primeiro handoff remove brief, designs, tarefas, grupo e conexões recém-criados em vez de deixar um fluxo parcial no Canvas.',
        'Respostas de agentes no Windows WSL continuam aguardando o transcript estruturado exato quando a PTY viva já conhece o id reservado da conversa mas a persistência via UNC ainda não terminou; ids obsoletos são reparados pela home local de cada provider sem nunca aceitar saída bruta da TUI.',
        'O pacote Windows inclui um runtime Node de console pinado e verificado por SHA-256. Os launchers nativo e WSL usam um processo de console real em vez do Deep Space.exe gráfico, preservando stdout, stderr, JSON-RPC do MCP e respostas confirmadas do ask; workspaces WSL existentes reparam o launcher sob demanda.',
        'Asks vinculados à tarefa expiram atrás de conversas antigas e são cancelados antes do envio quando a tarefa termina, é arquivada ou muda de responsável. Status atrasado não reabre trabalho concluído, avisos de conclusão também expiram e uma falha de despacho não desfaz uma tarefa concluída em paralelo.',
        ...
        'Visualizadores HTML do Cliente de API resolvem mídia e estilos relativos contra a URL da requisição sem credenciais em vez do servidor local do Deep Space, sob um sandbox limitado que bloqueia conteúdo ativo.',
    ...
    {
      date: '5 de setembro de 2026 · 0.25.0',
      title: 'Deep Space 0.25.0: workspace Remote completo em qualquer tela',
      summary: 'A navegação remota funciona no celular e cada agente agora tem um resumo operacional seguro do trabalho.',
      items: [
        'Equipe mostra o foco atual, a atividade semântica recente e totais limitados de mensagens enviadas, recebidas, respondidas e com falha sem expor mensagens internas nem a saída do terminal.',
    ...
    {
      date: '5 de setembro de 2026 · 0.24.2',
      title: 'Deep Space 0.24.2: entrega confirmada de mensagens no Windows',
      summary: 'Mensagens entre agentes agora iniciam um turno real do provider em vez de parar como texto colado no compositor do terminal.',
      items: [
        'Windows e WSL aguardam o redraw do compositor do TUI estabilizar antes de enviar Enter, impedindo que a tecla ultrapasse o processamento do texto colado.',
        'Um redraw do terminal não conta mais como entrega: o Deep Space confirma o prompt exato no transcript do provider e repete somente o Enter, dentro de um limite, quando necessário.',
        'Perguntas diretas, handoffs sem resposta, atribuições e conclusões do Kanban, roles, rotinas e revisões de design agora usam o mesmo caminho de entrega confirmada.',
        'Sessões WSL preservam a home de transcript da distribuição e o diretório Linux para confirmar a entrega e correlacionar a resposta exata.',
      ],
    },
    {
      date: '4 de setembro de 2026 · 0.24.1',
      title: 'Deep Space 0.24.1: dependências seguras e releases bloqueadas pela CI',
      summary: 'A base de dependências está segura novamente, o empacotamento dos terminais é determinístico e uma CI com falha não pode mais publicar uma release desktop.',
      items: [
        ...
        'STT e TTS embarcados agora usam sherpa-onnx-node 1.13.7 com os pacotes nativos oficiais correspondentes para macOS, Linux e Windows.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.24.0: mova seleções completas do Canvas entre workspaces',
    ...
    },
    {
      ...
      title: 'Deep Space 0.23.0: inteligência de código operacional e continuidade confiável',
      summary: 'Contexto de código compartilhado e atualizado se une a sessões resilientes, voz, abas de Portal e fluxos sem sobreposição.',
      items: [
        ...
        'Recarregar, trocar provider/runtime, dispensar, excluir, descarregar o workspace, desconectar o processo-pai ou fechar o app encerra todo o grupo do provider; um vínculo perdido é recuperado pelo id exato da conversa.',
        ...
        'Notas criadas por agentes via CLI ou MCP se conectam por padrão somente ao autor exato, mantendo fluxos criativos e de engenharia paralelos isolados até que um agente específico ou o time inteiro seja escolhido explicitamente.',
        ...
        'As conexões do canvas agora podem usar renderização Adaptativa, Elástica e animada ou Estática nas Configurações. Canvases adaptativos densos removem antes as simulações ociosas, enquanto Estático desativa toda física e animação das linhas para computadores menos potentes.',
        'A Inteligência de Código agora oferece os modos Assistido, Manual e Desativado por workspace. Assistido observa edições estabilizadas, atualiza os índices estrutural e semântico de forma incremental, reaproveita vetores inalterados e faz consultas dos agentes aguardarem a revisão conhecida mais recente; Manual mantém a indexação explícita para o usuário e bloqueia agentes; Desativado para watchers e acesso sem apagar o histórico local.',
        'O grafo mantém mouse, toque, wheel, canvases visuais e detecção do ponteiro alinhados tanto durante o zoom externo do Canvas quanto ao redimensionar o nó. Símbolos têm rótulos maiores que compensam a escala; o clique seleciona, centraliza e abre o contexto, o clique duplo aproxima e o clique duplo no vazio não aplica mais um zoom cego. Escopos de repositório, Impacto das alterações, busca lexical e busca semântica por intenção também explicam suas ações diretamente.',
        'Links de Portal que pedem uma aba em primeiro ou segundo plano agora criam outro nó Portal sem sobreposição no mesmo Canvas. Pop-ups reais de autenticação e pagamento continuam como janelas filhas isoladas para preservar window.opener e a sessão persistente.',
        'Sessões MCP e CLI restauradas sem DEEPSPACE_NODE_ID podem registrar o estado no Control Center por uma tarefa atribuída ao agente real do workspace; chamadas sem nenhuma das duas fontes verificadas de identidade continuam bloqueadas.',
        ...
        'O índice Semântico offline adiciona busca por intenção sem chave de API, download de modelo ou envio externo de código. O modo Assistido agora o mantém atualizado automaticamente; Manual preserva os controles explícitos Construir e Reconstruir.',
        ...
        'O scan do grafo lê arquivos suportados com concorrência limitada e as estatísticas da revisão separam scan, parse, resolução, persistência, cache hits, cache misses, arquivos alterados e estratégia de indexação. Raízes vazias não mantêm watcher recursivo; limites de recursos marcam o grafo como desatualizado e tentam novamente após um backoff limitado em vez de mostrar dados saudáveis.',
        'O benchmark reproduzível do grafo valida o repositório Deep Space, um espelho completo temporário e um repositório sintético contra as metas documentadas de busca, travessia e atualização incremental do SQLite; o espelho real de 1.672 arquivos permanece abaixo de dois segundos, portanto outro banco de grafo não se justifica.',
        'Instruções do workspace e de presets agora usam um bloco gerenciado e delimitado no AGENTS.md e, quando habilitado, no CLAUDE.md. O conteúdo existente do usuário é preservado, arquivos antigos controlados pelo Deep Space migram com segurança e desabilitar a sincronização remove somente esse bloco.',
        'A Inteligência de Código operacional explica a procedência das relações, sincroniza seleções do grafo e Monaco, sobrepõe agentes, tarefas e Andares ativos com conflitos de propriedade e monta pacotes revisados de 500 a 16.000 tokens para líder, agente, Council ou tarefa rastreável.',
        'Manifests compactos de revisões retidas permitem comparação estrutural, enquanto investigações nomeadas restauram projeto, visão, filtros, seleção, câmera e código aberto no Canvas, Workbench, CLI e MCP.',
        'Prévias de contexto limitado, handoffs operacionais e salvamento de investigações nomeadas agora aceitam seus payloads estritos sem tratar parâmetros de rota do workspace no Svelar como entrada inesperada do usuário.',
        'Ao voltar do Workbench, a Inteligência de Código restaura o mesmo projeto, visão, modo de busca, símbolo selecionado, opções de travessia e câmera do grafo em vez de reiniciar a investigação.',
        'Trabalho ativo mantém agentes desconectados com tarefas em andamento visíveis junto da tarefa, arquivos e símbolos, mas não permite escolhê-los para novos handoffs.',
      ],
    },
    {
      date: '28 ago 2026 · 0.22.0',
      title: 'Deep Space 0.22.0: fluxos nativos de imagem para pessoas e agentes',
      ...
      items: [
        ...
        'O executor Codex usa sua tool nativa autenticada image_gen.imagegen separadamente para cada output; referências seguem por referenced_image_paths e alpha inválido recebe edições corretivas limitadas.',
        'Um Codex conectado pode criar e configurar rascunhos, gerenciar entradas ordenadas de Notas e Imagens e executar um único run lógico com até dez outputs pelas tools tipadas image_workflow_*.',
        'O Deep Space nunca pede nem armazena chave de API de imagem e nunca chama diretamente um endpoint do provider. Somente o Codex atribuído conclui o run, e caminhos exatos, assinaturas, tamanhos e erros públicos são limitados antes da persistência.',
        'Os runs mantêm histórico e procedência rastreáveis, enquanto Canvas, Workbench, CLI/MCP e o tour de equipe criativa operam o mesmo fluxo visível sem um estado paralelo de automação.',
        'O caso de uso guiado agora monta e valida Personagem → Marca → Carrossel: cria briefings e logo de teste, aguarda outputs reais e conecta automaticamente cada resultado à etapa seguinte.',
        'O painel avisa que ImageGen exige uma conta ou assinatura autenticada do Codex, sem pedir uma chave de API da OpenAI.',
        'Agentes novos ou recuperados nunca retomam a conversa mais recente de outro terminal: o Deep Space retoma apenas o ID exato atribuído ao node e inicia uma conversa limpa quando esse vínculo ainda não existe.',
        'Terminais Codex interativos sempre recebem o MCP atual do app antes do resume, builds incluem o runtime WebADB completo e chamadas de fluxo de imagem deixam de falhar por MCP global antigo ou módulo de suporte Android ausente.',
        'Portais indisponíveis usam tentativas progressivas limitadas e retry manual; requests antigas do board retornam não encontrado; a limpeza ConPTY no Windows e probes locais deixam de poluir os logs com erros esperados.',
        'Os logs do desktop permanecem acionáveis: resultados esperados do Portal usam respostas estruturadas, a captura do microfone usa AudioWorklet, Electron e o atualizador usam APIs atuais e derivações Svelte inertes deixam de emitir avisos.',
        'Fluxos transparentes validam os pixels alpha do PNG decodificado. Com várias referências, o ImageGen primeiro compõe sobre um fundo branco uniforme e o remove em uma segunda edição nativa; o prompt de recorte de quadriculado já validado oferece outro fallback nativo, nunca Python ou processamento local de pixels. Runs parados expiram em vez de deixar o tour aguardando indefinidamente.',
        'A inicialização do app empacotado no macOS restaura a permissão executável do helper de PTY extraído, mantendo os terminais confiáveis após instalação de dependências ou extração de arquivos.',
      ],
    },
    {
      date: '27 ago 2026 · 0.21.2',
      title: 'Deep Space 0.21.2: colar confiável nos terminais do Windows',
      ...
      items: [
        ...
        'O conteúdo do clipboard permanece dentro do Electron e nunca é exposto pela ponte do renderer; somente a janela focada do Deep Space pode solicitar a colagem.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.21.1: workspaces permanecem pausados e o Windows fica em segundo plano',
      summary: 'O unload agora é persistente, e runtimes internos deixam de roubar o foco no Windows.',
      items: [
        'Descarregar um workspace persiste seu estado pausado e encerra terminais vivos, criação de sessões por tarefas e enfileiramento de automações em background, mantendo-o inativo entre navegações e reinícios até você abri-lo explicitamente.',
        'Workspaces pausados são identificados no Canvas e no Workbench, enquanto o layout e as conversas exatas dos agentes permanecem salvos para a próxima abertura explícita.',
        'Invocações internas da CLI e do servidor deixam de acionar o foco da instância única no Windows; abertura intencional do app, tray, notificações e links de colaboração continuam funcionando normalmente.',
      ],
    },
    {
      date: '26 ago 2026 · 0.21.0',
      title: 'Deep Space 0.21.0: Roles portáteis e criação mais rápida de workspaces',
      summary: 'Reuse Roles especialistas, escolha a conta certa do provider e arquive o workspace numa pasta durante a criação.',
      items: [
        'Adicionado o botão "Descobrir em outra pasta..." ao lado do discover do repositório já existente em Roles: escolha qualquer pasta num diálogo nativo e o Deep Space importa toda `role.json` encontrada dentro de `.deepspace/roles/` ali.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.20.1: configuração MCP segura no Codex',
      ...
      items: [
        'O Codex recebe as definições MCP do Deep Space e do Figma oficial por parâmetros temporários ao iniciar em runtime nativo ou WSL; o provisionamento deixa de reescrever ~/.codex/config.toml.',
        ...
        'AGENTS.md, arquivos MCP dos providers e opencode.json deixam de ser ocultados pelo .git/info/exclude; blocos legados exatos são reduzidos aos diretórios de runtime e skills pertencentes ao Deep Space.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.20.0: workspaces organizados e providers mais completos',
      ...
      items: [
        'Servidores MCP e skills adicionados ao workspace agora se propagam aos formatos nativos de Cursor, Cline, Devin, Antigravity e OpenCode, acompanhando os providers já cobertos pela ponte do Deep Space.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.19.0: contas, status, identidade dos providers e roteamento mais claro',
      ...
      items: [
        ...
        'O WebSocket de PTY aceita conexões do navegador somente pelo Deep Space na porta exata do app, impedindo que outro site em localhost abra ou controle sessões de terminal.',
      ],
    },
    {
      date: '24 ago 2026 · 0.18.1',
      title: 'Deep Space 0.18.1: estado confiável de projeto, Portal, voz e terminal',
      ...
      items: [
        'Os processos de terminal mantêm o ambiente do sistema operacional e a ponte do Deep Space, mas removem a APP_KEY do desktop e todas as variáveis privadas carregadas pelo runtime do app. Registros criptografados, cookies e sessões do Laravel passam a usar o .env do projeto e deixam de falhar com “The MAC is invalid”.',
        'Pop-ups de login dos Portais agora abrem numa janela sandboxed do Deep Space com a mesma sessão persistente, em vez de escapar para o browser do sistema. Cookies e storage são gravados em disco, e cada nó Portal restaura sua última URL navegada.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.18.0: coordenação durável, conhecimento com fontes e times reutilizáveis',
    ...
    },
    {
      ...
      title: 'Deep Space 0.17.0: autoria completa de testes de API para pessoas e agentes',
      ...
      items: [
        'A aba Testes agora alterna entre assertions estruturadas e um editor JavaScript de altura completa com autocomplete contextual para Bruno, Postman e Deep Space nativo. Scripts de teste executam separados da automação pós-resposta e fazem round-trip nos exports Bruno e Postman.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.16.0: runtimes oficiais de scripts Postman e Bruno',
    ...
    },
    {
      ...
      title: 'Deep Space 0.15.0: comandos reutilizáveis e Cliente de API multiprotocolo',
      ...
      items: [
        ...
        'Coleções de API agora podem ser exportadas para Bruno pelo serializador oficial ou para Postman v2.1 preservando metadados REST que o Deep Space não edita diretamente. Um formato versionado do Deep Space restaura todo o estado nativo, incluindo pastas, runners, ambientes, scripts e histórico.',
    ...
    },
    {
      ...
      title: 'Deep Space 0.14.0: pacotes RPM nativos para Linux',
      summary: 'Fedora, RHEL, CentOS e distribuições compatíveis agora têm um instalador nativo do Deep Space.',
      items: [
        ...
        'Os arquivos RPM usam o mesmo nome estável do produto Deep Space adotado pelos outros instaladores.',
    ...
    },
    {
      ...
      items: [
        'O Deep Space 0.13.0 mantém o teclado do terminal isolado dos atalhos de acessibilidade do Canvas. Escape chega corretamente ao Vim, editores de merge/rebase, pagers e outras TUIs sem desselecionar o node nem tirar o foco do xterm; busca e ditado continuam locais ao terminal.',
        ...
        'Terminais shell nativos agora preservam a pasta atual após reiniciar o Deep Space. Cursor e os demais providers também recebem uma tool explícita para listar notas existentes antes de ler ou editar, evitando duplicatas e arrays vazios.',
        ...
        'Documentos Design grandes agora expandem a área de trabalho ao redor de todos os frames em vez de cortar o que passa da página nominal. Use trackpad ou scroll, a ferramenta Mão (H), Espaço+arraste ou o botão do meio para navegar; Ajustar enquadra todo o conteúdo e o zoom chega a 2%. Exports e thumbnails usam os mesmos limites completos. Agentes conectados consultam design_reference uma vez, criam até 2.000 layers com design_create_elements e expandem a direção aprovada em lotes revisionados de design_apply_blueprint com no máximo 100 tokens, bindings/componentes ou itens de protótipo/motion. Explorações guiadas proíbem explicitamente inspecionar a instalação, fazer probes de schema ou criar scratch scripts de descoberta.',
        ...
        'O Modo Design nativo agora oferece variáveis de design tipadas em coleções e modos, aliases, vínculos pesquisáveis por propriedade, prévia imediata dos modos e o command bus visual completo e protegido por revisão para agentes via MCP do Deep Space.',
    ...
    },
    {
      ...
      items: [
        'O Deep Space 0.12.0 permite combinar o Windows nativo e múltiplas distribuições WSL no mesmo time. O workspace define o runtime padrão, cada terminal pode herdá-lo ou selecionar seu próprio ambiente, e detecção/modelos do provider, PTY, retomada, Council, recrutamento e ponte seguem o runtime efetivo. A troca reinicia somente o terminal afetado e valida distribuição, caminho e CLI sem fallback silencioso.',
        ...
        'Ao criar ou editar um workspace WSL, o Deep Space deriva e bloqueia automaticamente a pasta visível pelo Windows a partir do caminho Linux, sem exigir um segundo caminho equivalente nem rejeitar a configuração por engano.',
      ],
    },
    {
      ...
      items: [
        ...
        'O compartilhamento agora oferece convites separados para Navegador/celular e App Deep Space. O PWA Remote instalável acompanha agentes, tarefas, revisões, atividade e uso dos providers, persiste uma chave WebCrypto não extraível e remove o segredo do convite da URL antes de conectar.',
        'O compartilhamento de workspace agora usa por padrão o endpoint de produção relay.deepspace.app. O relay aceita a origem local dinâmica do app instalado e as origens web oficiais configuradas, enquanto rejeita sites não relacionados.',
        ...
        'Imagens, PDFs, arquivos e links agora podem ser soltos, colados ou selecionados em agentes, tarefas, notas e composers; arquivos de até 10 MB ficam confinados em .deepspace/attachments/ e seguem no briefing completo.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.8.3: o seletor pesquisável de modelos agora segue a composição oficial do shadcn-svelte, abre sem cortar a busca e mantém lupa, lista e foco alinhados.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.8.2: conversas entre Claude e Codex foram validadas nos dois sentidos com respostas reais e confirmação pelo transcript correto.',
    ...
    },
    {
      ...
      items: [
        ...
        'O Deep Space Light ganhou contraste consistente em painéis, nós, textos, botões, ícones, marcas de providers e hovers.',
    ...
    },
    {
      ...
      items: [
        ...
        'A ponte do Deep Space provisiona a configuração MCP e a skill do Devin, enquanto os transcripts ATIF entregam respostas limpas entre agentes e para o TTS.',
    ...
    },
    {
      ...
      items: [
        ...
        'O preset Deep Space Contributing combina Claude líder, oráculos Codex e Kimi, especialistas Svelar/desktop/QA e um Flow que exige consenso antes da criação das tarefas.',
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.4 é a primeira release para macOS com assinatura Developer ID Application e notarização da Apple; a assinatura ad-hoc fica restrita a builds locais.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.3 corrige o pacote macOS da 0.1.2: os arquivos estavam íntegros, mas a assinatura ad-hoc parcial fazia o Gatekeeper informar que o app estava danificado.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.2: o painel Usage agora atualiza Claude, Codex e Kimi automaticamente a cada 5 minutos, em vez de a cada 60 segundos, reduzindo chamadas desnecessárias e o risco de HTTP 429.',
    ...
    },
    {
      ...
      items: [
        'Deep Space 0.1.1 inclui electron-updater no aplicativo instalado; Configurações deixa de confundir módulo ausente com execução fora do app desktop.',
    ...
    },
    {
      date: '07 ago 2026',
      items: [
        'Deep Space 0.1.0: primeira release pública preparada para atualizar as instalações 0.0.1.',
    ...
    },
    {
      ...
      items: [
        ...
        'O handshake global do deepspace mcp agora inicia mesmo fora de um workspace; token e URL só são exigidos quando uma tool realmente acessa a ponte.',
        'Encerramento seguro de porta com confirmação, revalidação do PID e proteção do processo do Deep Space; portas arbitrárias da máquina nunca aparecem.',
    ...
    },
    {
      ...
      items: [
        ...
        'Fix: servidor MCP do Deep Space falava framing errado (LSP) e o Kimi dava timeout de 30s — agora é NDJSON, o padrão oficial do MCP (Claude, Kimi e cia conectam).',
        ...
        'deepspace list agora marca quem é o líder com [LIDER] — agentes não chutam mais "deepspace ask Maestro" (Maestro é o papel, não um título).',
      ],
    },
    {
      date: '04 ago 2026',
      items: [
        ...
        'Sem briga de portas entre workspaces: deepspace port devolve porta livre e os agentes aprendem a nunca matar processo de porta alheia.',
    ...
    },
    {
      date: '01 ago 2026',
      items: [
        'Nasce o Deep Space: canvas de agentes, ponte CLI, andares (worktrees), rotinas, roles, kanban, portal e Modo Maestro.',
