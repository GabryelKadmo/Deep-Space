<script lang="ts">
  ...
  const itemClass = (selected: boolean) =>
    `inline-flex h-7 flex-1 items-center justify-center gap-1.5 rounded-[5px] px-2.5 text-ui-sm font-medium transition-[background-color,color,box-shadow] duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--app-accent)] ${
      selected
...
<nav
  class="flex w-full items-center gap-0.5 rounded-md border border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-0.5"
