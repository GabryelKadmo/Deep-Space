...
<div class="flex h-full min-h-0 flex-col text-ui-sm">
  <div class="space-y-2 border-b border-[var(--app-border)] p-2">
    ...
    <div class="flex items-start gap-2 border border-[var(--app-border)] bg-[var(--app-surface-raised)] p-2 text-ui-xs leading-4 text-[var(--app-text-muted)]">
    ...
    {:else if status?.connected}
      <div class="flex items-center gap-2 border border-[var(--app-border)] p-2"><span class="grid size-6 shrink-0 place-items-center bg-[var(--app-success-soft)] text-[var(--app-success)]"><Check size={12} /></span><div class="min-w-0 flex-1"><p class="truncate font-medium text-[var(--app-text)]">{status.account?.handle}</p><p class="truncate text-ui-xs text-[var(--app-text-muted)]">{status.account?.email}</p></div><Button variant="ghost" size="icon-sm" class="size-6" title={m['design.figma_remove_credential']()} aria-label={m['design.figma_remove_credential']()} onclick={() => void removeCredential()}><KeyRound size={11} /></Button></div>
    {:else}
      <p class="text-ui-xs leading-4 text-[var(--app-text-muted)]">{desktop?.saveAutomationSecret ? m['design.figma_token_help']() : m['design.figma_desktop_required']()} {#if desktop?.saveAutomationSecret}<a class="text-[var(--app-accent)] hover:underline" href="https://help.figma.com/hc/en-us/articles/8085703771159-Manage-personal-access-tokens" target="_blank" rel="noreferrer">{m['design.figma_create_token']()}</a>{/if}</p>
  ...
  <div class="min-h-0 flex-1 overflow-y-auto">
    {#if status?.connected}
      <section class="space-y-2 border-b border-[var(--app-border)] p-2">
        <div class="text-ui-xs font-semibold uppercase text-[var(--app-text-muted)]">{m['design.figma_import']()}</div>
        ...
        {#if inspection}
          <div class="border border-[var(--app-border)]">
            <div class="border-b border-[var(--app-border)] px-2 py-1.5"><p class="truncate font-medium text-[var(--app-text)]">{inspection.fileName}</p><p class="text-ui-xs text-[var(--app-text-muted)]">{m['design.figma_file_summary']({ components: String(inspection.components), styles: String(inspection.styles) })}</p></div>
            <div class="max-h-40 overflow-y-auto p-1">
              {#each inspection.nodes as node (node.id)}<label class="flex min-h-8 items-center gap-2 px-1.5 py-1 hover:bg-[var(--app-surface-raised)]"><Checkbox checked={selectedNodeIds.includes(node.id)} onCheckedChange={(checked: boolean | 'indeterminate') => toggleNode(node.id, checked === true)} /><div class="min-w-0 flex-1"><p class="truncate text-ui-xs text-[var(--app-text)]">{node.name}</p><p class="truncate text-ui-xs text-[var(--app-text-muted)]">{node.type}{node.pageName ? ` · ${node.pageName}` : ''}{node.width && node.height ? ` · ${Math.round(node.width)} x ${Math.round(node.height)}` : ''}</p></div></label>{/each}
            </div>
          </div>
    ...
    <section class="space-y-2 border-b border-[var(--app-border)] p-2">
      <div><p class="text-ui-xs font-semibold uppercase text-[var(--app-text-muted)]">{m['design.figma_plugin_bridge']()}</p><p class="mt-1 text-ui-xs leading-4 text-[var(--app-text-muted)]">{m['design.figma_plugin_help']()}</p></div>
    ...
    {#if preview}
      <section class="border-t border-[var(--app-border)] p-2">
        <div class="mb-2 flex items-center justify-between"><div><p class="font-semibold text-[var(--app-text)]">{m['design.figma_sync_review']()}</p><p class="text-ui-xs text-[var(--app-text-muted)]">{preview.link.fileName}</p></div><Badge variant={pendingChanges.some((change) => change.state === 'conflict') ? 'destructive' : 'outline'}>{m['design.figma_changes_count']({ count: String(pendingChanges.length) })}</Badge></div>
        <div class="max-h-64 space-y-1 overflow-y-auto">
          {#if !pendingChanges.length}<p class="border border-dashed border-[var(--app-border)] p-3 text-center text-ui-xs text-[var(--app-text-muted)]">{m['design.figma_up_to_date']()}</p>{/if}
          {#each pendingChanges as change (change.nodeId)}<div class="grid grid-cols-[minmax(0,1fr)_100px] items-center gap-2 border border-[var(--app-border)] p-1.5"><div class="min-w-0"><p class="truncate text-ui-xs text-[var(--app-text)]">{change.name}</p><p class={`text-ui-xs ${change.state === 'conflict' ? 'text-[var(--app-danger)]' : 'text-[var(--app-text-muted)]'}`}>{stateLabel(change.state)}</p></div><NativeSelect.Root value={resolutions[change.nodeId] ?? change.defaultResolution} onchange={(event: Event) => (resolutions[change.nodeId] = (event.currentTarget as HTMLSelectElement).value as 'figma' | 'local' | 'delete')}><NativeSelect.Option value="figma">{m['design.figma_use_figma']()}</NativeSelect.Option><NativeSelect.Option value="local">{m['design.figma_keep_local']()}</NativeSelect.Option><NativeSelect.Option value="delete">{m['design.delete']()}</NativeSelect.Option></NativeSelect.Root></div>{/each}
        </div>
