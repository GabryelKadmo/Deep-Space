...
import { isPortFree, run } from '../../packages/deepspace-cli/src/cli.js';

/**
 * Testa a CLI `deepspace` contra um servidor HTTP falso que emula a bridge.
 */
describe('deepspace CLI', () => {
  let server;
  let apiUrl;
  let cwd;
  const requests = [];
  ...
  beforeAll(async () => {
    server = createServer((req, res) => {
      ...
      req.on('end', () => {
        requests.push({
          ...
          agentToken: req.headers['x-deepspace-agent-token'],
        });
        ...
        if (req.url === '/api/agent-room/bridge/git' && req.method === 'GET') {
        ...
        } else if (req.url?.startsWith('/api/agent-room/bridge/code-graph/contracts?')) {
          ...
          res.end(JSON.stringify({ data: { state: 'ready', model: 'deepspace-code-subword-v1', dimensions: 384, indexedSymbols: 80, totalSymbols: 80, builtAt: '2026-08-31T12:00:00.000Z' } }));
    ...
    cwd = mkdtempSync(join(tmpdir(), 'deepspace-cli-'));
    mkdirSync(join(cwd, '.deepspace'));
    writeFileSync(join(cwd, '.deepspace', 'workspace.json'), JSON.stringify({ token: 'tok123', apiUrl }));
  });
  ...
  it('consulta, revisa e executa operacoes Git rastreadas', async () => {
    const { lines, out } = capture();
    const env = { DEEPSPACE_NODE_ID: 'n1', DEEPSPACE_AGENT_TOKEN: 'terminal-token' };
  ...
  it('list usa a identidade automatica e diferencia portais conectados', async () => {
    const { lines, out } = capture();
    const code = await run(['list'], { cwd, out, env: { DEEPSPACE_NODE_ID: 'n1' } });
    expect(code).toBe(0);
  ...
  it('consulta e registra memoria com fonte explicita', async () => {
    ...
    const added = capture();
    expect(await run(['memory', 'add', 'Architecture', '--content', 'Use repositories', '--kind', 'decision', '--source-label', 'User direction'], { cwd, out: added.out, env: { DEEPSPACE_NODE_ID: '00000000-0000-7000-8000-000000000001' } })).toBe(0);
  ...
  it('consulta e percorre o mesmo grafo de codigo exposto no canvas', async () => {
    ...
    expect(await run(['graph', 'contracts', '--limit', '300', '--graph'], { cwd, out, env: {} })).toBe(0);
    expect(requests.at(-1).url).toContain('/code-graph/contracts?limit=300&includeGraph=true');
    expect(lines.join('\n')).toContain('Contracts: 1 endpoints');
  ...
  it('note create identifica o autor e nao compartilha com o time por padrao', async () => {
    const { out } = capture();
    await run(['note', 'create', 'Spec X'], { env: { DEEPSPACE_NODE_ID: 'n1' }, cwd, out });
  ...
  it('design apply envia operacoes e revisao sem editar o arquivo diretamente', async () => {
    ...
    ], { env: { DEEPSPACE_NODE_ID: 'designer-1' }, cwd, out });
  ...
  it('design layout e typography limitam propriedades e usam o command bus', async () => {
    ...
    ], { env: { DEEPSPACE_NODE_ID: 'designer-1' }, cwd, out })).rejects.toThrow('Propriedades invalidas');
  });
  ...
  it('task done envia a identidade e mostra o handoff ao lider', async () => {
    const { lines, out } = capture();
    await run(['task', 'done', 't1'], { env: { DEEPSPACE_NODE_ID: 'codex-node' }, cwd, out });
  ...
  it('sem token retorna erro claro', async () => {
    const emptyDir = mkdtempSync(join(tmpdir(), 'deepspace-cli-empty-'));
    await expect(run(['list'], {
      cwd: emptyDir,
      out: () => {},
      env: { DEEPSPACE_TOKEN: '', DEEPSPACE_RUNTIME_FILE: '' },
    })).rejects.toThrow('Token');
  });

  it('MCP completa o handshake mesmo fora de um workspace Deep Space', async () => {
    const emptyDir = mkdtempSync(join(tmpdir(), 'deepspace-mcp-empty-'));
    const input = new PassThrough();
    const chunks: string[] = [];
    const done = run(['mcp'], {
      cwd: emptyDir,
      env: { DEEPSPACE_RUNTIME_FILE: '' },
      input,
    ...
    expect(response.result.serverInfo.name).toBe('deepspace');
  });

  it('env DEEPSPACE_TOKEN tem precedencia sobre arquivo', async () => {
    const { lines, out } = capture();
    await run(['list'], { cwd, out, env: { DEEPSPACE_TOKEN: 'env-tok', DEEPSPACE_API_URL: apiUrl } });
  ...
  it('keeps the workspace bridge config when an agent changes into a scratch directory', async () => {
    const scratch = mkdtempSync(join(tmpdir(), 'deepspace-agent-scratch-'));
    const configPath = join(cwd, '.deepspace', 'workspace.json');
    const { lines, out } = capture();

    const code = await run(['list'], {
      cwd: scratch,
      out,
      env: { DEEPSPACE_WORKSPACE_CONFIG: configPath },
    });
  ...
  it('does not silently replace an invalid configured workspace file', async () => {
    const scratch = mkdtempSync(join(tmpdir(), 'deepspace-agent-scratch-invalid-'));
    ...
    await expect(run(['list'], {
      cwd,
      out: () => undefined,
      env: { DEEPSPACE_WORKSPACE_CONFIG: invalidConfig },
    })).rejects.toThrow('Config da ponte invalido');
  });

  it('recruit usa DEEPSPACE_NODE_ID como --from padrao', async () => {
    const { out } = capture();
    await run(['recruit', 'Dev Frontend', '--provider', 'claude', '--model', 'opus', '--effort', 'high', '--floor', 'floor-1'], { env: { DEEPSPACE_NODE_ID: 'n1' }, cwd, out });
  ...
  it('flag --from explicito tem precedencia sobre o env', async () => {
    const { out } = capture();
    await run(['recruit', 'QA', '--from', 'Outro'], { env: { DEEPSPACE_NODE_ID: 'n1' }, cwd, out });
    const request = requests.filter((entry) => entry.url === '/api/agent-room/bridge/recruit').at(-1);
    expect(request.body.from).toBe('Outro');
  });

  it('list usa DEEPSPACE_NODE_ID como --agent padrao', async () => {
    const { out } = capture();
    await run(['list'], { env: { DEEPSPACE_NODE_ID: 'n1' }, cwd, out });
    expect(requests.at(-1).url).toContain('agentNodeId=n1');
  });
  ...
  it('runtime.json tem precedencia sobre o apiUrl do workspace.json', async () => {
    ...
    const staleDir = mkdtempSync(join(tmpdir(), 'deepspace-cli-stale-'));
    mkdirSync(join(staleDir, '.deepspace'));
    writeFileSync(join(staleDir, '.deepspace', 'workspace.json'), JSON.stringify({ token: 'tok123', apiUrl: 'http://127.0.0.1:1' }));
    const runtimeFile = join(staleDir, 'runtime.json');
    writeFileSync(runtimeFile, JSON.stringify({ apiUrl }));

    const { lines, out } = capture();
    const code = await run(['list'], { cwd: staleDir, out, env: { DEEPSPACE_RUNTIME_FILE: runtimeFile } });
    expect(code).toBe(0);
    expect(lines.join('\n')).toContain('Claude');
  });
  ...
  it('api lista e executa requests salvos com variaveis', async () => {
    const { lines, out } = capture();
    expect(await run(['api', 'list'], { cwd, out, env: { DEEPSPACE_NODE_ID: 'n1' } })).toBe(0);
    expect(lines.join('\n')).toContain('GET Health');
    expect(requests.at(-1).url).toContain('agentNodeId=n1');

    expect(await run(['api', 'run', 'api1', 'r1', '--variables', '{"baseUrl":"https://example.test"}'], { cwd, out, env: { DEEPSPACE_NODE_ID: 'n1' } })).toBe(0);
    expect(requests.at(-1)).toMatchObject({
      method: 'POST',
  ...
  it('api importa, sincroniza, cria, le, substitui e exporta colecoes completas pela mesma bridge do MCP', async () => {
    ...
    const { lines, out } = capture();
    const env = { DEEPSPACE_NODE_ID: 'n1' };
    ...
    expect(requests.at(-1).body).toMatchObject({ action: 'push', resolution: 'deepspace', from: 'n1' });
  ...
  it('port devolve uma porta livre (sem precisar de workspace.json)', async () => {
    const emptyDir = mkdtempSync(join(tmpdir(), 'deepspace-cli-port-'));
    const { lines, out } = capture();
