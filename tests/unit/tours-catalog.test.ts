...
function createdTitles(tour: Tour): Set<string> {
  ...
  for (const step of tour.steps) {
    ...
    for (const action of actions) {
      if (action.kind === 'createAgent' || action.kind === 'createNote' || action.kind === 'createUsage' || action.kind === 'createApiClient' || action.kind === 'createCodeGraph' || action.kind === 'createImageWorkflow' || action.kind === 'createSampleImage' || action.kind === 'createShape' || action.kind === 'createDesign' || action.kind === 'createConsole') titles.add(action.title);
