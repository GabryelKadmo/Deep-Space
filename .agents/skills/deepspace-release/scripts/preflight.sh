...
VERSION="${1:-}"
MODE="${2:-new}"
SOURCE_REPO="GabryelKadmo/Deep-Space"
...
[[ "$(git remote get-url origin)" == *"GabryelKadmo/Deep-Space"* ]] || fail 'origin is not GabryelKadmo/Deep-Space'
...
printf 'Release preflight passed for DeepSpace %s (%s mode).\n' "$VERSION" "$MODE"
