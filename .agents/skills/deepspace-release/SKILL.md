---
name: deepspace-release
description: Prepare, publish, recover, and audit DeepSpace desktop releases for macOS, Windows, and Linux. Use when asked to cut or publish a new DeepSpace version, retry a failed release, inspect the Release Desktop workflow, validate public installers or latest-*.yml feeds, or diagnose desktop auto-update delivery.
---
...
Read `.agents/skills/deepspace-release/references/release-contract.md` when checking artifacts, signing behavior, or failure recovery.
...
- for the one-time `0.1.4` feed transition, add/update the three localized changelogs and README guidance in sibling `../deepspace-releases`;
...
```bash
bash .agents/skills/deepspace-release/scripts/preflight.sh <version> new
```
...
```bash
git tag -a v<version> -m "DeepSpace v<version>"
...
```bash
bash .agents/skills/deepspace-release/scripts/preflight.sh <version> recover
git tag -fa v<version> -m "DeepSpace v<version>" HEAD
...
```bash
node .agents/skills/deepspace-release/scripts/audit-release.mjs <version>
```
